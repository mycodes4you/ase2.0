...
if (!isset($_SESSION['usuario'])) {
	...
	if($resultado == '1' || $_SESSION['rol12']=='1' || $_SESSION['rol13']=='1' || $_SESSION['rol14']=='1') {
...
//	if($renglones > '7') { $ren = $renglones; } 
	...
	$matr1 = mysql_query($preg1) or die("ERROR: Fallo selección de cuentas!");
	...
	echo '			</form>'."\n";
	echo '			<br>
								<a href="contabilidad.php"><img src="idiomas/es_MX/imagenes/regresar.png"></a>';
...
elseif($accion==='lista_polizas') {
	...
	$preg0 = "SELECT * FROM " . $dbpfx . "cont_polizas ORDER BY poliza_num DESC";
	$matr0 = mysql_query($preg0) or die("ERROR: Fallo selección de cuentas!");
	//$pol = mysql_fetch_array($matr0);
	$fila = mysql_num_rows($matr0);
	$msj = 'No se localizó la póliza indicada';
	...
	if($fila > 0) {
		echo '			<table cellpadding="2" cellspacing="1" border="1" class="agrega" width="800">';
		echo '				<tr class="cabeza_tabla"><td colspan="2">Listado de Polizas</td></tr>'."\n";
		echo '				<tr><td colspan="2" style="text-align:left;">
					<table class="izquierda" cellpadding="2" cellspacing="2" border="0" width="100%">'."\n";
			echo '						<tr>
							<td>#</td>
							...
							<td>Ciclo</td>
							<td>Tipo</td></tr>'."\n";
		while($pol = mysql_fetch_array($matr0)) {
			echo '						<tr class="' . $fondo . '">
							<td>' . $pol['poliza_num'] . '</td>
							<td><a href="contabilidad.php?accion=consultarpoliza&ciclo='.$pol['poliza_ciclo'].'&poliza='.$pol['poliza_num'].'">' . $pol['poliza_descripcion'] . '</a></td>
							<td>' . $pol['poliza_factura'] . '</td>
							<td>' . $pol['poliza_estatus'] . '</td>
							<td>' . $pol['poliza_fecha'] . '</td>
							<td>' . $pol['poliza_ciclo'] . '</td>
							<td>';
...
elseif($accion==='consultarpoliza') {
	...
	echo '						<tr class="cabeza_tabla"><td style="text-align:center;">Cuenta Contable</td><td style="text-align:center;">Debe</td><td style="text-align:center;">Haber</td></tr>'."\n";
		...
		while($mov = mysql_fetch_array($matr1)) {
			$preg3 = "SELECT * FROM " . $dbpfx . "proveedores WHERE cuenta_contable = '".$mov['lib_cuenta']."'";

		 //OR cuenta_cont_aux1 = '".$mov['lib_cuenta']."' OR cuenta_cont_aux2 = '".$mov['lib_cuenta']."' OR cuenta_cont_aux3 = '".$mov['lib_cuenta']."'";
			$matr3 = mysql_query($preg3) or die ("ERROR: Fallo seleccion de cuenta proveedor");
			$prov = mysql_fetch_assoc($matr3);
			//echo $preg3;
			//echo $prov['prov_razon_social'];
			$preg4 = "SELECT * FROM " . $dbpfx . "cont_cuentas WHERE cuenta_contable = '".$mov['lib_cuenta']."'";
			$matr4 = mysql_query($preg4) or die ("ERROR: Fallo seleccion de cuenta proveedor");
			$cuenta = mysql_fetch_assoc($matr4);
			//echo $preg4;
			...
			$matr2 = mysql_query($preg2) or die("ERROR: Fallo selección de cuentas!");
			$cat = mysql_fetch_array($matr2);

			/*if($cat['nombre_contable'] == ''){$nombre_contable = $prov['prov_razon_social'];}
			elseif($cat['nombre_contable'] != ''){$nombre_contable = $cat['nombre_contable'];}*/
			
			if(is_null($cat['nombre_contable']) AND $mov['lib_tipo'] == '0') {$nombre_contable = $prov['prov_nic'];}
			if(is_null($cat['nombre_contable']) AND $mov['lib_tipo'] == '1') {$nombre_contable = $cuenta['ban_nombre'] .' - '. $cuenta['ban_cuenta'];}
			elseif($cat['nombre_contable'] != ''){$nombre_contable = $cat['nombre_contable'];}
			//echo $cat['nombre_contable'];

			echo '						<tr class="' . $fondo . '"><td style="text-align:left;"><strong>' . $mov['lib_cuenta'] . '</strong> ' . $nombre_contable . '</td>'."\n";
		...
		echo '						<div class="obscuro espacio">
							...
							<form action="contabilidad.php?accion=consultarpoliza" method="post">
								<table cellpadding="0" cellspacing="0" border="0">
									...
									<tr><td colspan="2" style="text-align:left;"><a href="contabilidad.php?accion=lista_polizas">Listado de Polizas</a></td></tr>
