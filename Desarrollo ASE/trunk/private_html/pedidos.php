...
if($accion==='listar') {
	...
	if(validaAcceso('1050000', $dbpfx) == '1' || ($solovalacc != 1 && ($_SESSION['rol02']=='1' || $_SESSION['rol13']=='1' || $_SESSION['rol08']=='1'))) {
		// Acceso autorizado
	} else {
...
//	echo $preg0 . '<br>';
	...
	if($fila0 > 0) {
		...
		if($ped['pedido_estatus'] == 10 && (validaAcceso('1050005', $dbpfx) == '1' || ($solovalacc !='1' && ($_SESSION['rol08']=='1')))) {
		...
		if($ped['pedido_qv'] > 0) {
			echo '<img src="imagenes/logo-quien-vende-48.png" width="48" alt="Quien-Vende.com"><br>'."\n";
		}
...
// ------ Inicio de la tabla de productos del pedido ------
		...
		$fondo = 'claro';
		echo '		<form action=';
		if($ped['pedido_estatus']==0 && (validaAcceso('1050020', $dbpfx) == '1' || ($solovalacc != '1' && ($_SESSION['rol08']=='1')))) {
...
// ------ Aquí se determina si se muestra o no el renglón al momento de imprimir --------
			...
			if($ped['pedido_estatus']==5 || $ped['pedido_estatus']==7) {
				...
				} elseif((validaAcceso('1050022', $dbpfx) == '1' || ($solovalacc != 1 && ($_SESSION['rol02']=='1' || $_SESSION['rol08']=='1'))) && $prod['op_recibidos'] == 0 && ($fila4 > 0 || $pagdelped > 0)) {
...
//						if($pag['pago_tipo'] == '4') { $pag['monto'] = $pag['monto'] * -1; }
		...
		if($pend_x_fact > 0 || $fila4 < 1) {
			...
			if($ped['pedido_estatus'] >= 5 && $ped['pedido_estatus'] <= 30 && ($_SESSION['rol02']=='1' || $_SESSION['rol13']=='1') && $factdoc == '0' && ($ped['pedido_tipo'] == '2' || $ped['pedido_tipo'] == '3')) {
				...
				if($ped['pedido_estatus'] < 20) {
					echo '<a href="pedidos.php?accion=pagar&pedido_id=' . $pedido . '&orden_id=' . $ped['orden_id'] . '&por_pagar=' . $pend_x_fact .'&adelanto=1&provid=' . $ped['prov_id'] . '"><button type="button">' . $lang['PagoAdelan'] . '</button></a>';
				}
			}
			echo '</td>'."\n";
			echo '			</table></form><hr>'."\n";
...
//					echo 'paso 5 <br>';
		...
		if($qv_activo == 1 && $canxml != '') {
			...
			$xml .= '               </OT>'."\n";
			$xml .= '       </Comprador>'."\n";
			$xmlnom = $nick . '-' . $orden_id . '-' . date('YmdHis') . $mtime . '.xml';
...
//			echo $i . ' -> ' . $parametros . ' ' . $recibir[$i] . '<br>';
			if($quitar[$i] < '1' && $recibir[$i] > 0) {
				...
				if($comcost['sub_orden_id'] == '999999997' || $pedido_tipo == '9' ) {
					...
					$preg_adeudos = "SELECT * FROM " . $dbpfx . "prods_pendientes WHERE prod_id = '" . $comcost['prod_id'] . "' AND prods_pendiente_adeudos > 0";
					//echo $preg_adeudos . '<br>';
					$matr_pendientes = mysql_query($preg_adeudos) or die("ERROR: en selección de prods_pendientes! " . $preg_adeudos);
					$adeudos = mysql_num_rows($matr_pendientes);
					if($adeudos > 0) {
						while($con_adeudos = mysql_fetch_array($matr_pendientes)) {
							if($disp2 >= $con_adeudos['prods_pendiente_adeudos']){ // --- Si nueva disponibilidad alcanza para surtir se procede ---
								...
								$nvadispo = $disp2 - $con_adeudos['prods_pendiente_adeudos'];

							} elseif($disp2 < $con_adeudos['prods_pendiente_adeudos']){ // ---- Surtir las disponibles ---

								$surtido_actualizado = $con_adeudos['prods_pendiente_surtidos'] + $disp2;
								$adeudos_actualizado = $con_adeudos['prods_pendiente_adeudos'] - $disp2;
								unset($sql_data);
								$sql_data = [
									'prods_pendiente_surtidos' => $surtido_actualizado,
						...
						$parametros = "prod_id = '" . $comcost['prod_id'] . "' ";
...
elseif($accion==="cancelafact") {
	...
	if ($error === 'no') {
		...
		$result = actualiza_pedido($pedido_id, $dbpfx);
//		echo 'Monto Pagos: ' . $result;
		redirigir('pedidos.php?accion=consultar&pedido=' . $pedido_id);
	} else {
