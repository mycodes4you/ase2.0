...
/*  ----------------  obtener nombres de proveedores   ------------------- */
		...
		$consulta = "SELECT aseguradora_id, aseguradora_nic FROM " . $dbpfx . "aseguradoras";
...
//		$provs[0] = 'Sin Proveedor';
		while ($ase = mysql_fetch_array($arreglo)) {
			$aseg[$ase['aseguradora_id']] = $ase['aseguradora_nic'];
		}
		$aseg[0] = 'Particular';
...
$preg = "SELECT orden_id FROM " . $dbpfx . "ordenes WHERE (orden_estatus < '90' OR orden_estatus = '99') AND orden_fecha_recepcion >= '" . $feini . "' AND orden_fecha_recepcion <= '" . $fefin . "' AND orden_id >= '" . $orden . "'";
