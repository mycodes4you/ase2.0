...
// ------ Obtener nombres e identificadores de Flotillas o Convenios ---
$pregase = "SELECT aseguradora_id, aseguradora_nic, aseguradora_logo FROM " . $dbpfx . "aseguradoras WHERE aseguradora_activa = '1'";
$matrase = mysql_query($pregase) or die("Error: Falló selección de aseguradoras! " . $pregase);
while($ase = mysql_fetch_array($matrase)) {
	$aseg[$ase['aseguradora_id']] = ['nic' => $ase['aseguradora_nic'], 'logo' => $ase['aseguradora_logo']];
}

if (($accion==="crear" ) || ($accion==="modificar")) {

	if (validaAcceso('1145005', $dbpfx) == '1' || ($solovalacc != 1 && ($_SESSION['rol06'] == '1')) ) {	
...
//	echo 'Estamos en la sección crear';
	if($error=='no') {
		if($_SESSION['exp']['placas'] != '') {
			...
			$_SESSION['vehiculo']['eco'] = $_SESSION['exp']['eco'];
	...
	<table cellpadding="0" cellspacing="0" border="0" class="agrega">
		...
		<tr><td><strong>* ' . $lang['Flota'] . '</strong></td>
			<td style="text-align:left;"><select name="flota" size="1">
				<option value="">' . $lang['Selecciona'] . '</option>
				<option value="0">' . $lang['Particular'] . '</option>'."\n";
		foreach($aseg as $ka => $va) {
			echo '				<option value="' . $ka . '" ';
				if(($_SESSION['vehiculo']['flota'] != '' && $_SESSION['vehiculo']['flota'] == $ka) || ($_SESSION['vehiculo']['flota'] == '' && $vehiculo['vehiculo_flota'] == $ka)) { echo ' selected="selected" '; }
				echo '">' . $va['nic'] . '</option>'."\n";
		}
		echo '			</select></td>
		</tr>
		<tr><td><strong>* ';
		if($vehiculo['vehiculo_flota'] > 0) {
			echo $lang['NumEco'];
		} else {
			echo $lang['Placas'];
		}
		echo '</strong></td><td><input type="text" name="placas" size="60" maxlength="60" value="'; 
		...
		echo '</strong></td><td><input type="text" name="eco" size="60" maxlength="60" value="';
		echo ($vehiculo['vehiculo_eco'] != '') ? $vehiculo['vehiculo_eco'] : $_SESSION['vehiculo']['eco']; 
		echo '" /></td></tr>'."\n";
...
// ------ Cierre de ASEBase de datos comunes -----------------------------		
			...
			echo '		<tr><td><strong>* ' . FABRICANTE . '</strong>' . $_SESSION['vehiculo']['marca'] . '</td><td style="text-align:left;"><input type="text" name="marca" size="60" maxlength="40" value="';
			echo ($vehiculo['vehiculo_marca'] != '') ? $vehiculo['vehiculo_marca'] : $_SESSION['vehiculo']['marca'];
			echo '" /></td></tr>'."\n";
			echo '		<tr><td><strong>* ' . TIPO . '</strong>' . $_SESSION['vehiculo']['tipo'] . '</td><td style="text-align:left;"><input type="text" name="tipo" size="60" maxlength="40" value="'."\n";
	...
	//caracteristicas
		...
		echo '		<tr><td colspan="2" style="text-align:left;"><input type="submit" value="Enviar" name="Enviar"/>&nbsp;<input type="reset" name="limpiar" value="Borrar" /></td></tr>
	</table>
	</form>';
	} else {
		echo $mensaje;
...
elseif (($accion==='insertar') || ($accion==='actualizar')) {
	...
	$eco = strtoupper($eco); $_SESSION['vehiculo']['eco'] = $eco;
...
//	if (strlen($tipo) < 2) {$error = 'si'; $mensaje .='El nombre del Tipo (submarca) es muy corto: ' . $tipo . '<br>';}
	if (strlen($color) < 3 && $docingreso == 1) {$error = 'si'; $mensaje .='El nombre del color es muy corto: ' . $color . '<br>';}
	if (strlen($modelo) < 4 || strlen($modelo) > 4) {$error = 'si'; $mensaje .='El año debe tener 4 números (por ejemplo 2012): ' . $modelo . '<br>';}
	if ($marca == '0') {$error = 'si'; $mensaje .='Seleccione una Marca de Vehículo.<br>';}
	if ($tipo == '0') {$error = 'si'; $mensaje .='Seleccione un Tipo de Vehículo.<br>';}
...
*/
	...
	if($accion==='insertar') {
		...
		$matr = mysql_query($preg) or die("ERROR: Fallo selección de vehiculos! L432" . $preg);
...
//		print_r($sql_data_array);
      if ($accion==='insertar') {
		...
		$ord = mysql_fetch_array($matr0);
     	...
     	unset($_SESSION['vehiculo']);
     	if($marca == 'OTRA MARCA' || $tipo == '0' || $tipo == 'OTRO TIPO') {
			bitacora($ord['orden_id'], 'No se encontró Marca o Tipo para este Vehículo.', $dbpfx, 'No se encontró Marca o Tipo para este Vehículo.', 3, '', '', 701);
     	}
     	$cliente_id = '';
     	if($regexp == 1) {
...
//	echo 'Estamos en la sección  consulta';
	if (validaAcceso('1145010', $dbpfx) == '1' || $_SESSION['codigo'] < '60'  || $_SESSION['codigo'] > '70' ) {	
// Acceso autorizado
	} else {
...
//		echo $pregunta;
	...
	if ($num_cols == 1 && $error == 'no') {
		...
		while ($vehiculo = mysql_fetch_array($matriz)) {
			...
			echo '		<tr class="cabeza_tabla"><td colspan="2" align="left">' . $lang['Datos del vehículo'] . ': ' . $vehiculo['vehiculo_marca'] . ' ' . $vehiculo['vehiculo_tipo'] . ' ' . $vehiculo['vehiculo_color'] . ' ' . $vehiculo['vehiculo_modelo'] . ' ';
			if($vehiculo['vehiculo_flota'] > 0) {
				echo $lang['NumEco'];
			} else {
				echo $lang['Placas'];
			}
			echo ': ' . $vehiculo['vehiculo_placas'] . '</td></tr>
		<tr><td colspan="3">' . $lang['Propiedad del Cliente'] . ': ' . $cli['cliente_nombre'] . ' ' . $cli['cliente_apellidos'] . '. Tel: ' . $cli['cliente_telefono1'] . '. Movil: ' . $cli['cliente_movil'] . '</td></tr>'."\n";
			...
			if (validaAcceso('1145000', $dbpfx) == '1' || ($solovalacc != 1 && ($_SESSION['rol06']==='1'))) {
			...
			echo '		<tr><td>' . $lang['Flota'] . '</td><td>' . $aseg[$vehiculo['vehiculo_flota']]['nic'] . '</td></tr>'."\n";
			echo '		<tr><td>' . $lang['Número de vehículo'] . '</td><td>' . $vehiculo['vehiculo_id'] . '</td></tr>'."\n";
			echo '		<tr><td>' . $lang['Activo'] . '?</td><td>';
			...
			echo			 '</td><td>' . $vehiculo['vehiculo_eco'] . '</td></tr>
		...
		<tr><td>' . $lang['Cilindrada'] . '</td><td>' . $vehiculo['vehiculo_litros'] . '</td></tr>'."\n";
			...
			if ($retorno == '1' || ($solovalacc != 1 && ($_SESSION['rol06']==='1'))) {
				echo '		<tr><td colspan="2">
			<a href="vehiculos.php?accion=modificar&vehiculo_id=' . $vehiculo['vehiculo_id'] . '"><img src="idiomas/' . $idioma . '/imagenes/cambiar-datos.png" alt="Modificar" title="Modificar"></a> 
			<a href="personas.php?accion=consultar&cliente_id=' . $vehiculo['vehiculo_cliente_id'] . '"><img src="idiomas/' . $idioma . '/imagenes/datos-de-clientes.png" alt="Ver Datos del Cliente" title="Ver Datos del Cliente"></a> 
			<a href="previas.php?accion=consultar&vehiculo_id=' . $vehiculo['vehiculo_id'] . '"><img src="idiomas/' . $idioma . '/imagenes/presupuesto-previo.png" alt="Ver o Crear Presupuesto Previo" title="Ver o Crear Presupuesto Previo"></a> 
