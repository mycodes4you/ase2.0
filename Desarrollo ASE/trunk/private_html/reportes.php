...
// echo 'del ' . $feini . ' al ' . $fefin . ' ---<br>';
	...
	if(isset($feini) && $feini != '0000-00-00') {
		...
		//if($feini != '0000-00-00' || $feini == ''){
		$feini = date('Y-m-01 00:00:00');
		//}
		//else {
		//	$feini = $_POST['feini2'];
		//}
		//if($fefin != '0000-00-00' || $fefin == ''){
...
//	$f = validaAcceso('', $dbpfx); if($f == '1') { $accpf = 1; }
	...
	if($accpf == '1' || ($solovalacc != '1' && ($_SESSION['rol02']=='1' || $_SESSION['rol03'] == '1' || $_SESSION['rol04'] == '1' || $_SESSION['rol06'] == '1' || $_SESSION['rol12'] == '1'))) {
...
if($export != 1) { // ---- Hoja de calculo ----
	...
	if ($accion==="reportes" || $accion==="entregados" || $accion==="seguimiento" || $accion==="desglose" || $accion==="destajo" || $accion==="operadores" || $accion==="finanzas" || $accion==="cliente" || $accion==="manodeobra" || $accion==="audatrace" || $accion==="ajustadores" || $accion==="comentreg" || $accion==="refproceso" || $accion==="valuaciones" || $accion==="facturacion" || $accion==="comentarios" || $accion==="vigilancia" || $accion==="deducibles" || $accion==="ingext" || $accion==="ingresos" || $accion === "carga_operador" || $accion === "encuesta" || $accion==="carga_operador" || $accion==="cumplimiento_operadores" || $accion === "detalle_siniestro" || $accion === "utilidad_area" || $accion === "pagos_grupales") {
		...
		if ($f1125100 == '1' || $_SESSION['rol02']=='1' || $_SESSION['rol03'] == '1' || $_SESSION['rol12'] == '1') {
			echo '					<div><a href="reportes.php?accion=facturacion&nomrep=Facturación"><img src="idiomas/' . $idioma . '/imagenes/r_facturacion.png" alt="Facturación" title="Facturación"></a></div>'."\n";
		}
		if ($f1125100 == '1' || $_SESSION['rol02']=='1' || $_SESSION['rol03'] == '1' || $_SESSION['rol12'] == '1') {
			echo '					<div><a href="reportes.php?accion=pagos_grupales&nomrep=Pagos Grupales"><img src="idiomas/' . $idioma . '/imagenes/pagos_grupales.png" alt="Pagos Grupales" title="Pagos Grupales"></a></div>'."\n";
		}
		...
		echo '					<tr><td style="vertical-align:top;"><b><big>Fecha de Inicio</big></b><br>';
		...
		echo '</td><td style="vertical-align:top;"><b><big>Fecha de Fin</big></b><br>';
		//instantiate class and set properties
		...
		if($accion === 'finanzas' || $accion === 'deducibles' || $accion === 'utilidad_area') {
			echo '<b>Filtra por Estatus</b><br>';
		...
		if($accion === 'ingresos' && $flujoflt == '1') {
			...
			echo '<br><b>Filtra por Cliente</b><br>';
			echo '							<select name="asegflt" size="1" onchange="document.filtrorep.submit()";>'."\n";
			echo '								<option value=""';
			if($asegflt == '') { echo ' selected '; }
			echo '>Todos los Clientes</option>'."\n";
		...
		/* °°°°°°°°°°° SE PUEDE ELIMINAR YA QUE ESTÁ REPETIDO EN LA LINEA 508 °°°°°°°°°°°°°°°°°°°°
		elseif($accion == 'reportes' || $accion == 'finanzas' || $accion === 'utilidad_area' || $accion == 'ingresos' || $accion == 'comentreg') {
			echo '<b>Filtra por Cliente</b><br>';
			echo '							<select name="asegflt" size="1" onchange="document.filtrorep.submit()";>'."\n";
			echo '								<option value=""';
			if($asegflt == '') { echo ' selected '; }
			echo '>Todos los Clientes</option>'."\n";
		...
		*/
        ...
        if($accion == 'reportes'){
			...
			$href = 'reportes.php?accion=facturacion&export=1&feini='.$feini.'&fefin='.$fefin.'&cliente=' . $cliente . '&aseguradora_id=' . $aseguradora_id . '&factura=' . $factura . '&ot=' . $ot . '&fltcli=' . $fltcli . '&fact_monto=' . $fact_monto . '&t_ini=' . $t_ini . '&t_fin=' . $t_fin .'&fact_tipo=' . $fact_tipo . '&rangoF=' . $_POST['rangoF'];
		} elseif($accion == "pagos_grupales"){
			$href = 'reportes.php?accion=pagos_grupales&export=1&feini='.$feini.'&fefin='.$fefin.'&cliente=' . $cliente . '&aseguradora_id=' . $aseguradora_id . '&factura=' . $factura . '&ot=' . $ot . '&fltcli=' . $fltcli . '&fact_monto=' . $fact_monto . '&t_ini=' . $t_ini . '&t_fin=' . $t_fin .'&fact_tipo=' . $fact_tipo . '&rangoF=' . $_POST['rangoF'];
		...
		if($accion == 'reportes' || $accion == 'comentreg' || $accion == 'deducibles' || $accion === "destajo" || $accion == "finanzas" || $accion == "ingresos" || $accion == "facturacion" || $accion == 'manodeobra' || $accion == "cliente" || $accion == "refproceso" || $accion == "cumplimiento_operadores" || $accion == "valuaciones" || $accion == "utilidad_area"){
			echo '
					</td>'."\n";
			echo '
					<td>
	...
	$mes_este = date('n');
	//	echo date('Y-m-t');
	$preg0 = "SELECT orden_id, orden_cliente_id, orden_estatus, orden_categoria, orden_servicio, orden_vehiculo_placas, orden_vehiculo_tipo, orden_vehiculo_color, orden_vehiculo_id, orden_alerta, orden_asesor_id, orden_fecha_recepcion, orden_fecha_proceso_inicio, orden_fecha_proceso_fin, orden_fecha_de_entrega, orden_fecha_promesa_de_entrega, orden_fecha_ultimo_movimiento, orden_grua FROM " . $dbpfx . "ordenes WHERE ";
	...
	//echo $preg0;
	$matr0 = mysql_query($pregn) or die("ERROR: Fallo selección de lapso!");
...
if ($accion==="tabla") {
	...
	if ($f1125005 == '1' || ($solovalacc != '1' && ($_SESSION['rol02']=='1' || $_SESSION['rol03'] == '1' || $_SESSION['rol04'] == '1'))) {
	...
	if ($f1125005 == '1' || $f1125010 == '1' || $_SESSION['rol02']=='1' || $_SESSION['rol03'] == '1' || $_SESSION['rol04'] == '1' || $_SESSION['rol06'] == '1') {
		...
		echo '				<tr class="cabeza_tabla"><td colspan="13" style="text-align: right;">' . $filas . $encabezado . '</td></tr>' . "\n";
		echo '				<tr><td>';
		if($confolio == 1 && $id == 17) {
			echo 'Provisional';
		} else {
		...
		echo '</td><td>Vehículo</td><td>' . $lang['Placa'] . '</td><td>Categoría de Servicio</td><td>Tipo de Servicio</td><td>Cliente</td><td>Siniestro</td><td>Grúa</td><td>Estatus</td><td>';
		if($ordenar == '' || !isset($ordenar)) {
			echo '<a href="' . $_SERVER['REQUEST_URI'] . '&ordenar=asesor">Asesor</a>';
		...
		echo '</td><td>';
		if(isset($of) || isset($osf)) {
			echo 'Fecha Entregado';
		} else {
			echo 'Fecha Recibido';
		}
		echo '</td><td>' . $lang['FechaPromesa'] . '</td><td>Días en proceso</td></tr>' . "\n";
		$fondo = 'claro';
		$j = 0;
		$hoy = strtotime(date('Y-m-d 23:59:59'));
		while($ord = mysql_fetch_array($matr0)) {
			...
			if($oculta == 'no') {
				...
				echo '<td>' . constant('ORDEN_SERVICIO_' . $ord['orden_servicio']) . '</td>';
				...
				echo '</td>';
				echo '<td style="text-align:center;">' . $lang_grua[$ord['orden_grua']] .'</td>';
				...
				echo '</td><td style="padding-left:10px; padding-right:10px;">' . date('Y-m-d', strtotime($ord['orden_fecha_recepcion'])) . '</td><td style="padding-left:10px; padding-right:10px;">' . $fpe . '</td><td align="center">'.$dias.'</td>
				</tr>';
...
if ($accion==="entregados") {
	...
	//	echo $preg0;
	$matr1 = mysql_query($preg1) or die("ERROR: Fallo selección de lapso!");
	...
	while($ord = mysql_fetch_array($matr1)) {
		...
		$matr2 = mysql_query($preg2) or die("ERROR: Fallo selección de lapso!");
		$fila2 = mysql_num_rows($matr2);
		echo '				<tr class="' . $fondo . '">
...
if ($accion==="pagos_grupales") {
	...
	//---- Include del reporte ----
    include('parciales/reportes/pagos_grupales.php');
}
...
if ($accion==="encuesta") {
	...
	if($f1125125 == '1' || $_SESSION['rol02']=='1' || $_SESSION['rol03']=='1'){
		...
		if($detalle == 1){
		$encabezado = "Detalles del " .  date('Y-m-d', strtotime($feini)) . " al " . date('Y-m-d', strtotime($fefin));
//------------ CONSULTA DETALLE DE PREGUNTA -----------------
		$con_preg = "SELECT AS content * FROM " . $dbpfx . "pregs_encuesta WHERE preg_id = '" . $preg . "'";
...
//--------------- CALCULAMOS EL PROMEDIO TOTAL DE LA PREGUNTA ---------------
			...
			$total_prom = round(((($prom / $total) * 100) / $num_resp),2);
			echo '
			<div style=" width:95%; background-color:#c0c0c0; border-radius: 10px; float:center; box-shadow: 5px 5px 10px #000; margin: 0 auto;">
				 <dl>'."\n";
...
if ($accion==="detalle_siniestro"){ // --- accesible solo por URL ---
	
	//---- Include del reporte ----
    include('parciales/reportes/detalle-siniestro.php');
