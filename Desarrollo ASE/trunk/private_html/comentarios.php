...
elseif ($accion==='visto') {
	...
	$preg0 = "UPDATE " . $dbpfx . "comentarios SET fecha_visto = '" . date('Y-m-d H:i:s') . "' WHERE bit_id = '" . $visto . "'";
	...
	$preg1 = "SELECT u.nombre, u.apellidos, u.email, c.comentario, c.orden_id FROM " . $dbpfx . "comentarios c, " . $dbpfx . "usuarios u WHERE c.usuario = u.usuario AND c.bit_id = '" . $visto . "'";
	$matr1 = mysql_query($preg1) or die("ERROR: Fallo selección de Usuarios !" . $preg1);
	$usu = mysql_fetch_array($matr1);

	if($usu['orden_id'] == '9999995') {
		redirigir('proveedores.php?accion=actstatus&prov_id=' . $prov_id . '&activar=' . $activar);
	}

			if($usu['email'] != '') {

				$preg2 = "SELECT c.fecha_visto, u.nombre, u.apellidos FROM " . $dbpfx . "comentarios c, " . $dbpfx . "usuarios u WHERE c.para_usuario = u.usuario AND c.bit_id = '" . $visto . "'";
