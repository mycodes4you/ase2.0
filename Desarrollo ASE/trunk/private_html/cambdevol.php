				...
				<div class="row"> <!-box header del título. -->
					<div class="col-sm-12">
						<table class="table" style="width:100%;">
							<thead><tr>
								<th class="cen">Req ID / OT</th>
								<th>Vehículo</th>
								<th>' . $lang['DetalleSol'] . '</th>
								<th>' . $lang['Solicitante'] . '</th>
								<th>Motivo</th>
								<th>' . $lang['Devolucion'] . '</th>
								<th>' . $lang['Reemplazo'] . '</th>
								<th>' . $lang['Rechazar'] . '</th>
							</tr></thead>
							<tbody>'."\n";
		...
		while($ele = mysql_fetch_array($matr2)) {
			...
			echo '
							<tr class="' . $fondo . '">
								...
								<td>' . $veh['completo'] . '</td>
								<td>' . $tipo_cd . ' de ' . $ele['cantidad'] . ' ' . $ele['nombre'] . '</td>
								<td class="cen">' . $usu[$cds['usu_responsable']]['nombre'] . ' ' . $usu[$cds['usu_responsable']]['apellidos'] . '</td>
								<td>' . $cds['motivo'] . '</td>
								<td class="cen"><input type="radio" name="dictamen[' . $ele['elem_id'] . ']" value="1" /></td>
								<td class="cen"><input type="radio" name="dictamen[' . $ele['elem_id'] . ']" value="2" /></td>
								<td class="cen"><input type="radio" name="dictamen[' . $ele['elem_id'] . ']" value="3" /></td>
							</tr>'."\n";
...
elseif($accion === 'dictaminar') {
	...
	if(validaAcceso('1175005', $dbpfx) == '1' || $_SESSION['usuario'] == $usudictcd || ($solovalacc != 1 && ($_SESSION['rol02']=='1'))) {
		// Acceso autorizado
	} else {
		$_SESSION['msjerror'] = $lang['AccNoAut'];
		redirigir('almacen.php');
...
// ------ 8.- Localiza el mensaje interno enviado al dictaminador y lo da como resuelto.
				...
				ejecutar_db($dbpfx . 'orden_productos', $sqldata, 'actualizar', $param);
				if($qv_activo == 1) {
					$opxml .= '                     <Ref op_id="' . $prod['op_id'] . '" op_estatus="57" fecha_recibido="' . date('Y-m-d H:i:s') . '" />'."\n";
				}
...
// ------ Localizar los mensajes de aviso al dictaminador y cerrarlos en automático ----
			...
			if($qv_activo == 1 && $opxml != '') {
				...
				$xml .= '				<OT orden_id="' . $ele['orden_id'] . '" marca="' . $veh['marca'] . '" tipo="' . $veh['tipo'] .
 '" color="' . $veh['color'] . '" vin="' . $veh['serie'] . '" modelo="' . $veh['modelo'] .'" foto_frontal="' . $veh['foto_frontal'] .'" foto_izquierda
