...
//			echo 'Copia y Pega desde AUDATEX las <span style="color:#f00; font-weight:bold;">PIEZAS SUSTITUIDAS que tienen precio</span>:';
			...
			echo ' y Precio al Público (si no sabe el precio colocar un # en lugar del precio)</span></big><br>
								EJEMPLO:<br>
								<b>FASCIA TRASERA KL12345678 $1,250.50</b>'."\n";
		} else {
						echo '
	...
	if($sub_orden['sub_area'] <='6' || $sub_orden['sub_area'] >='8') {
		...
		if($sub_orden['sub_siniestro']==='1') {
			...
			echo '		
					<tr>
						<td colspan="2" style="text-align:left;">
							Agrega aquí la Mano de Obra requerida: <br>
							...
							<input type="text" name="preciounidad" value="' . $preciout . '" />
						</td>
					</tr>'."\n";
	...
	$preg0 = "SELECT op_id, op_item_seg, op_codigo, op_nombre, op_cantidad, op_precio, op_tangible, prod_id, op_pedido, op_surtidos FROM " . $dbpfx . "orden_productos WHERE sub_orden_id = '" . $sub_orden['sub_orden_id'] . "' AND op_pres IS NULL AND op_tangible < '3' ORDER BY op_tangible, op_nombre";
	...
	while($op = mysql_fetch_array($matr0)) {
		...
		if($sub_orden['fact_id'] > 0 || $op['op_pedido'] > 0 || ($op['op_tangible'] == '0' && $sub_orden['recibo_id'] > 0)) {
			echo $op['op_cantidad'];
			echo '
										<input type="hidden" name="cantp[' . $cuenta . ']" value="' . $op['op_cantidad'] . '" />'."\n";
		} else {
			echo '
										<input class="valua" style="text-align:right;" type="text" name="cantp[' . $cuenta . ']" value="" size="2" placeholder="' . $op['op_cantidad'] . '" />'."\n";
		}
		echo '</td>
									<td>' . $op['op_nombre'] . '</td>
									<td>' . $op['op_codigo'] . '</td>
		...
		if($sub_orden['fact_id'] > 0 || ($op['op_tangible'] == '0' && $sub_orden['recibo_id'] > 0)) {
			...
			echo '
										<input type="hidden" name="precio[' . $cuenta . ']" value="' . $op['op_precio'] . '" />';
		} else {
			echo '
										<input class="valua" style="text-align:right;" type="text" name="precio[' . $cuenta . ']" value="' . number_format($op['op_precio'],2) . '" size="6" />'."\n";
		}
		echo '							
										<input type="hidden" name="op_pedido[' . $cuenta . ']" value="' . $op['op_pedido'] . '" />
										<input type="hidden" name="op_id[' . $cuenta . ']" value="' . $op['op_id'] . '" /></td>
									<td>'."\n";

		if($op['prod_id'] > 0){ // --- Producto de un paquete de servicio ---
			
			if($op['op_surtidos'] > 0){ // --- El producto ya fue entregado al operador ---
				echo '
										Entregado';
			} else{
				echo '
										<input type="checkbox" name="borrar[' . $cuenta . ']" value="1" />'."\n";
			}
			...
			if($op['op_pedido'] > 0 ) {
				echo '
										<a href="pedidos.php?accion=consultar&pedido=' . $op['op_pedido'] . '" target="_blank">P ' . $op['op_pedido'] . '</a>'."\n";
			} elseif($op['op_item_seg'] > 0 ) {
				echo '
										<a href="refacciones.php?accion=gestionar&orden_id=' . $sub_orden['orden_id'] . '#' . $op['op_id'] . '" target="_blank">E ' . $op['op_item_seg'] . '</a>'."\n";
			} elseif($op['op_tangible'] == 0 && $op['recibo_id'] > 0 ) {
				echo '
										<a href="recibosrh.php?accion=consultar&recibo_id=' . $op['recibo_id'] . '" target="_blank">RD ' . $op['recibo_id'] . '</a>'."\n";
			} else {
				echo '
										<input type="checkbox" name="borrar[' . $cuenta . ']" value="1" />'."\n";
			}
...
*/
	...
	echo '			<input type="hidden" name="sub_orden_id" value="' . $sub_orden_id . '" />
			...
			<input type="hidden" name="siniestro" value="' . $sub_orden['sub_reporte'] . '" />
...
elseif(($accion==='avaluo') || ($accion==='mod_avaluo')) {
	...
	if($determina_area == 1 && ($area == 1 || $area == 6)) { // -- $determina_area def. Valores ---
...
// Obtener precio de parte
		...
		$descripcion = limpiar_cadena($descripcion);
		$audaprod[$i][0] = $descripcion;
		$audaprod[$i][1] = trim($precioar);
		$audaprod[$i][2] = trim($estructural);
		$audaprod[$i][3] = trim($codigo);
		$audaprod[$i][4] = trim($cant);

		if($determina_area == 1 && ($area == 1 || $area == 6)) {
			// --- Si $determina_area es agual a 1 y el area en curso es hojalatería o mecanica , se aplica el algoritmo para determinar a que área pertenece
			$audaprod[$i][5] = determina_area($descripcion);
		} else{
			$audaprod[$i][5] = $area;
		}
	}
	...
	foreach ($audacons2 as $i => $v) {
		...
		if($cant == '') { $cant = '0'; }
		$descripcion = limpiar_cadena($descripcion);
		$audaconsumible[$i][0] = $descripcion;
	...
	foreach ($audamo2 as $i => $v) {
		...
		$descripcion = limpiarString($descripcion);
		if($precioar == '') { $precioar = '0';}
		$audaobr[$i][0] = $descripcion;
		if($particular == '1') {$precioar * 10;}
		$audaobr[$i][1] = trim($precioar);
		
		if($determina_area == 1  && ($area == 1 || $area == 6)){ // --- si la variable no ha sido modificada en la tabla valores y el area en curso es hojalatería o mecanica , se aplica el algoritmo de determinación
			$audaobr[$i][5] = determina_area($descripcion);
	...
	if($determina_area == 1 && ($area == 1 || $area == 6)) {
		...
		// ---- REABRIMOS CONEXION LA BASE DE DATOS DE LA INSTANCIA ---- 
	...
	if(($error === 'no') && (is_array($prod_id) || isset($paquete) || is_array($_SESSION['prods']['id']) || is_array($audaprod) || is_array($audaobr) || is_array($audap) || is_array($op_id) || is_array($audaconsumible) )) {
		...
		if (is_array($op_id)) {
			foreach($op_id as $i => $ii) {
				...
				if(isset($borrar[$i]) && $borrar[$i] == '1') {
					$pregp = "SELECT op_id, prod_id, op_cantidad, op_nombre, op_recibidos FROM " . $dbpfx . "orden_productos WHERE op_id = '" . $op_id[$i] . "' AND op_pedido < '1' AND op_surtidos < '0.0001'";
					$matrp = mysql_query($pregp);
					$regr = mysql_fetch_array($matrp);
					if($regr['prod_id'] > 0) {
						$suma = "UPDATE " . $dbpfx . "productos SET prod_cantidad_disponible = prod_cantidad_disponible + " . $regr['op_recibidos'] . " WHERE prod_id = '" . $regr['prod_id'] . "'";
					...
					if($qv_activo == 1 && $asurt[$aseguradora_id] == '1' && $regr['op_id'] == $op_id[$i]) {
				...
				if($prod_cantidad[$i]!='') {
					...
					if($prod_disponible[$i] >= $prod_cantidad[$i]) {
						...
						$sql_data_array = array(
							'op_reservado' => $prod_disponible[$i],
				...
				if(($audaobr[$i][0]!='') && ($audaobr[$i][1]!='')) {
					...
					if($audaobr[$i][5] != $area && $audaobr[$i][5] > 0) {
						...
						if(isset($audaobr[$i][5]) && $audaobr[$i][5] == 0) {
							// --- Si el área fue 0 se agrega a la lista de refacciones no identificadas ---
							mysql_close(); // --- cerramos conexion actual --- 
							mysql_connect("localhost", $dbusuario, $dbclave) or die ('Falló la conexion a la DB. 1107');
							mysql_select_db("ASEBase") or die ("Base de datos ASEBase no encontrada.");
		...
		if($qv_activo == 1 && isset($xmlitem) && $asurt[$aseguradora_id] == '1') {
		...
		if($determina_area == 1 && is_array($prods_faltantes) && ($area == 1 || $area == 6)) {
			...
			foreach($prods_faltantes as $key => $val) {
				...
				$item++;
				$nueva_id = ejecutar_db($dbpfx . 'orden_productos', $sql_data_array3, 'insertar');
				if($val['op_tangible'] == 1 && $qv_activo == 1 && $asurt[$aseguradora_id] == '1') {
					// --- Cotizar refacciones ---
...
// ------ Captura de Refacciones ------
	echo '						<tr><td>
								<div class="row">
									<div class="col-md-12 panel-body" style="text-align: left !important;">
										<legend class="legend"><span style="font-weight:bold;">' . $lang['REFACCIONES'] . ': ' . $sub_siniestro . '</span></legend>
									</div>
								</div>
							</td></tr>'."\n";
