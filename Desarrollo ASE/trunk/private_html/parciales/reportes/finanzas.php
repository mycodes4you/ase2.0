...
if ($f1125030 == '1' || $_SESSION['rol02']=='1') {
	...
	if($export == 1){ // ---- Hoja de calculo ----
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue("C4", "OT")
					->setCellValue("D4", "Vehículo")
					->setCellValue("E4", $lang['Placa'])
					->setCellValue("F4", "Estatus")
					->setCellValue("G4", "Cliente")
					->setCellValue("H4", $lang['Siniestro'])
					->setCellValue("I4", "Venta de Ref")
					->setCellValue("J4", "Venta de Materiales")
					->setCellValue("K4", "Venta de MO")
					->setCellValue("L4", "Monto de Venta")
					->setCellValue("M4", "Costo Aseguradora")
					->setCellValue("N4", "Fact/Rem")
					->setCellValue("O4", "Monto facturado")
					->setCellValue("P4", "Monto por facturar")
					->setCellValue("Q4", "Cobrada")
					->setCellValue("R4", "Descuentos")
					->setCellValue("S4", "Costo Ref")
					->setCellValue("T4", "Costo Mats")
					->setCellValue("U4", "Costo TOT")
					->setCellValue("V4", "Costo Destajo")
					->setCellValue("X4", "Costo Total")
					->setCellValue("Y4", "% Utilidad");
		$z= 5;
		...
		while($gsub = mysql_fetch_array($matr2)) {
			...
			if($oculta == 'no'){
				...
				if($export == 1){ // ---- Hoja de calculo ----
					...
					$u = 'U'.$z; $v = 'V'.$z; $x = 'X'.$z; $y = 'Y'.$z;
				
					$datos_vehi = strtoupper($ord['orden_vehiculo_tipo']) . ' ' . strtoupper($ord['orden_vehiculo_color']);
					
					$f_recepcion = date('Y-m-d', strtotime($ord['orden_fecha_recepcion']));
					$f_recepcion = PHPExcel_Shared_Date::PHPToExcel( strtotime($f_recepcion) );
					
					$mes_recepcion = date('m', strtotime($ord['orden_fecha_recepcion']));
					//$mes_recepcion = PHPExcel_Shared_Date::PHPToExcel( strtotime($mes_recepcion) );
					...
					$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue($a, $mes_recepcion)
								->setCellValue($b, $f_recepcion)
								->setCellValue($c, $ord['orden_id'])
								->setCellValue($d, $datos_vehi)
								->setCellValue($e, $ord['orden_vehiculo_placas'])
								->setCellValue($f, constant('ORDEN_ESTATUS_' . $ord['orden_estatus']))
								->setCellValue($g, $ase[$gsub['sub_aseguradora']][1]);
				...
				if($gsub['sub_reporte'] == '0' || $gsub['sub_reporte'] == '') {
					...
					if($export == 1){ // ---- Hoja de calculo ----
						...
						$objPHPExcel->setActiveSheetIndex(0)
									->setCellValue($h, $gsub['sub_reporte']);
				...
				if($export == 1){ // ---- Hoja de calculo ----
					...
					$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue($i, $partes)
								->setCellValue($jota, $cons)
								->setCellValue($kkk, $mo)
								->setCellValue($l, $pres);
				...
				while($fact = mysql_fetch_array($matr1)) {
					if($fact['fact_tipo'] < '3') {
						// --- Verificar si la factura está "cobrada" con una nota de crédito (cancelada) ---
						$pregnc = "SELECT cf.monto, c.cobro_tipo FROM " . $dbpfx . "cobros_facturas cf, " . $dbpfx . "cobros c WHERE cf.cobro_id = c.cobro_id AND cf.fact_id = '" . $fact['fact_id'] . "'";
						$matrnc = mysql_query($pregnc) or die("Falló selección de cobros " . $pregnc);
						$montonc = 0;
						while($cobnc = mysql_fetch_array($matrnc)) {
							if($cobnc['cobro_tipo'] == '4') {
								$montonc = $montonc + $cobnc['monto'];
							}
						}
						// --- Si el monto de la factura es mayor a lo cobrado mediante Nota de Crédito, se continua agregando información, de otra manera se ignora. ---
						if($fact['fact_monto'] > $montonc) {
							...
							$matr_ajuste = mysql_query($preg_ajuste) or die("Falló selección de ajustes " . $preg_ajuste);
							...
							// --- Consultar montos de las tareas facturadas ---
				...
				if($export == 1){ // ---- Hoja de calculo ----
					...
					$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue($n, $fact_num)
								->setCellValue($o, $total_facts_sin_iva)
								->setCellValue($p, $monto_no_fact)
								->setCellValue($q, $cobrada)
								->setCellValue($r, $aju_sin_iva)
								->setCellValue($s, $costref)
								->setCellValue($t, $costcons)
								->setCellValue($u, $costtot)
								->setCellValue($v, $costdest)
								->setCellValue($x, $totcosts);
