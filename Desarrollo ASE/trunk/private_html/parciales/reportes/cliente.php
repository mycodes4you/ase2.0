...
// ----- Si el filtro de cliente en el reporte de aseguradora viene colocado ---
	...
	if($a != '') {
		$ase_filtro[$a][0] = $ase[$a][0];
		$ase_filtro[$a][1] = $ase[$a][1];
		$ase_filtro[$a][2] = $ase[$a][2];
		unset($ase);
		$ase = $ase_filtro;
	}
		...
		foreach($ase as $k => $v) {
			$fondo = 'claro';
			$j = 0;
			$pregpo = "SELECT s.orden_id FROM " . $dbpfx . "subordenes s, " . $dbpfx . "ordenes o  WHERE s.sub_aseguradora = '" . $k . "' AND o.orden_id = s.orden_id AND s.sub_estatus < '190' AND ";

			if($tipo_reparacion == 'reparados') {
				$pregpo .= " (o.orden_estatus <= '30' OR o.orden_estatus = '99') AND ";
			} elseif($tipo_reparacion == 'no_reparados'){
				$pregpo .= " (o.orden_estatus >= '30' AND o.orden_estatus <= '98' AND o.orden_estatus != '90')  AND ";
			}
			...
			if($estatusflt == 1) {
				...
				$pregpo .= " o.orden_fecha_recepcion >= '" . $feini . "' AND o.orden_fecha_recepcion <= '" . $fefin . "' GROUP BY s.orden_id ORDER BY o.orden_fecha_recepcion";
			}
...
//			echo $pregpo . '<br>';
			...
			while($gpo = mysql_fetch_array($matrpo)) {
				...
				while ($ord = mysql_fetch_array($matr3)) {
					...
					$preg4 = "SELECT sub_partes, sub_consumibles, sub_mo FROM " . $dbpfx . "subordenes WHERE orden_id = '" . $ord['orden_id'] . "' AND sub_reporte = '" . $ord['sub_reporte'] . "' AND sub_estatus < '190' ";
					...
					if($export == 1) { // ---- Hoja de calculo ----
						...
						$objPHPExcel->setActiveSheetIndex(0)
									...
									->setCellValue($i, $cost_cons)
									->setCellValue($j, $cost_mo)
