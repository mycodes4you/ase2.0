...
if ($f1125100 == '1' || $_SESSION['rol02']=='1' || $_SESSION['rol12']=='1') {
	...
	$preg0 = "SELECT fact_id, orden_id, reporte, cliente_id, aseguradora_id, fact_rfc_emisor, fact_rfc_receptor, fact_num, fact_fecha_emision, fact_tipo, fact_monto, fact_cobrada, fact_fecha_cobrada, usuario FROM " . $dbpfx . "facturas_por_cobrar WHERE (fact_tipo < '3' OR fact_tipo = '4') ";
	...
	if($export == 1){ // ---- Hoja de calculo ----
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue("I4", "RPE")
					->setCellValue("J4", "RFC Emisor")
					->setCellValue("K4", "Nota de Crédito");
		...
		<div class="col-md-12">
			<div class="content-box-header">
				<div class="panel-title">';
					if($rangoF == 'on'){
						echo '<h3>' . $filas . $encabezado . '</h3>';
					}
					else{
						echo '<h2>' . $filas . $encabezado . '</h2>';
					}
	  		echo '		
				</div>
			</div>
		...
		<div class="col-md-12 ">
			<div id="content-tabla">
				<table cellspacing="0" class="table-new">
					<tr>
						<th><big>Factura</big></th><!-- RfcAlternos -->
						<th><big>OT</big></th>
					...
					<tr class="claro">
						...
						if ($accion === "facturacion") {
							echo '<br><label class="container" style="padding-left: 40px;">
							<input type="checkbox" ';
	...
	while($fact = mysql_fetch_array($matr0)) {
		...
		if($export == 1){ // ---- Hoja de calculo ----
			...
			$objPHPExcel->setActiveSheetIndex(0)
						...
						->setCellValue($i, $rpe)
						->setCellValue($j, $fact['fact_rfc_emisor'])
						->setCellValue($ke, $contenido);
