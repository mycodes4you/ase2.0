...
//	echo $estatus_cerrado . ' -> ' . $preg1;
	...
	if($export == 1) { // ---- Hoja de calculo ----
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue("N4", "Asesor de Servicio")
					->setCellValue("O4", "Presupuestador");
		$z= 5;
...
<div class="page-content">
	...
	while($ord = mysql_fetch_array($matr1)) {
		// --- Obtenemos el nombre de la persona que paso la OT de Presupuesto a Cotizando y del Asesor asignado ---
		$presusu = cambioEstatus($ord['orden_id'], '27', $dbpfx);
		$usupres = $usuario[$presusu[0]['usuario']];
		$asesor = $usuario[$ord['orden_asesor_id']];
		...
		while($sub = mysql_fetch_array($matr3)) {
			...
			if($export == 1){ // ---- Hoja de calculo ----
				...
				$objPHPExcel->setActiveSheetIndex(0)
							...
							->setCellValue('N'.$z, $asesor)
							->setCellValue('O'.$z, $usupres);
