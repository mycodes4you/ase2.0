...
$pdf->Image(DIR_DOCS . $qrnom[0] . '.png', 166, 26, 33, 33, 'PNG');
...
if($asegu['omite_datos_pdf'] != '1') {
	if($reporte != '' && $reporte != '0') {
		if($veh['flota'] != ''){
			$datos_veh = 'CLIENTE: ' . utf8_decode($clie['cliente_nombre']) . ' ' . utf8_decode($clie['cliente_apellidos']) . "\n";
		} else{
			$datos_veh = 'POLIZA: ' . $poliza . '. SINIESTRO: ' . $reporte . "\n" . 'CLIENTE: ' . utf8_decode($clie['cliente_nombre']) . ' ' . utf8_decode($clie['cliente_apellidos']) . "\n";	
		}
	}
	if($veh['flota'] != '') {
		$datos_veh .= 'VEHICULO : ' . $veh['marca'] . ' ' . $veh['tipo'] . ' ' . $veh['modelo'] . ' NUM.ECO: '  . $veh['eco'] . ' PLACAS: ' . $veh['placas'] . "\n";
	} else {
