...
*/
	...
	function bitacora($orden_id, $estatus, $dbpfx, $comentario, $interno, $sub_orden_id, $previa_id, $parausr) {
		if($previa_id != '') {
		...
		if ($comentario!='') {
			$query = "insert into " . $dbpfx . "comentarios (`bit_id`,`orden_id`,`interno`,`comentario`,`usuario`,`sub_orden_id`,`para_usuario`) VALUES ";
			$query .= "('" . $bit_id . "','" . $orden_id . "','" . $interno . "','" . $comentario . "','" . $_SESSION['usuario'] . "','" . $sub_orden_id . "','" . $parausr . "')";
			$result = mysql_query($query) or die($query);
		}
	}
...
//	header('Location: https://' . $host . '/' . $url);
	...
	function lista_documento ($orden_id, $estatus, $dbpfx, $presel, $previa_id) {
		global $codigomon;
		...
		<tr><td>Nombre</td><td>Archivo</td><td>Fecha de registro</td><td>Usuario</td><td>Vista previa</td>';
			if (validaAcceso('1025030', $dbpfx) == '1' || $_SESSION['rol05']=='1' || $_SESSION['rol06']=='1') {
				...
				echo '<form action="documentos.php?accion=depurar&orden_id=' . $orden_id . '" method="post" enctype="multipart/form-data" name="partidas"><input type="checkbox" name="presel" value="1" ';
...
<!--			<td>' . constant('ORDEN_ESTATUS_' . $documento['doc_etapa']) . '</td> -->';
			...
			if (validaAcceso('1025030', $dbpfx) == '1' || $_SESSION['rol06']=='1' || $_SESSION['rol05']=='1') {
				echo '<tr><td colspan="6" style="text-align:left;"><input type="submit" name="enviar" value="Eliminar" /><label>Si marcó documentos para eliminar, al presionar "Eliminar" serán eliminados. | <input type="submit" name="enviar" value="Descargar" /></label><input type="hidden" name="orden_id" value="' . $orden_id . '"/><input type="hidden" name="estatus" value="' . $estatus . '"/></td></tr>';
			}
			echo '	</table></form><br>'."\n";
		} else {
