...
//		  	$sql_data_array['orden_presupuesto'] = $pres_orden;
		...
		if($status_orden < 99) {
	   	...
	   	if ($orden['orden_ref_pendientes']=='0' && $status_orden > 2 && $status_orden < 12 && is_null($orden['orden_fecha_ref_recibidas'])) {
			...
			bitacora($orden_id, 'Se llamo a ajusta_orden pero no hubo cambio de estatus.', $dbpfx);
		}
	}
	...
	function ajusta_suborden ($orden_id, $area, $dbpfx) {
		$pregunta2 = "SELECT sub_estatus FROM " . $dbpfx . "subordenes WHERE orden_id = '" . $orden_id . "' AND sub_area = '" . $area . "' AND sub_estatus < '112'";
//		echo $pregunta2;
