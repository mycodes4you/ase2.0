...
// ------ Reinicia la protección contra doble envío ------
		...
		<div class="col-md-12">
	  		<div class="content-box-header" style="min-height: 60px;">
				<div class="panel-title">
		  			<span><big><?php echo ucfirst($accion); ?> Tarea: <b><?php echo $sub_orden_id;?></b> de la OT: <b><?php echo $sub_orden['orden_id'];?></b></big></span>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="col-lg-8 col-md8 col-sm-8" style="margin: 5px;">
		<div style="box-shadow: 3px 4px 10px 0px #484848; border-radius: 10px; margin-bottom: 10px;">
			<div class="row">
				<div class="col-lg-2"></div>
				<div class="col-lg-8" style="margin: 10px; background-color: white" align="center"><img src="<?php echo constant('ASEGURADORA_' . $sub_orden['sub_aseguradora']); ?>" alt=""></b></big></span>
				</div>
				<div class="col-lg-2"></div>
			</div>
			...
			<div class="row">
				<div class="col-lg-2"></div>
				<div class="col-lg-8" style="margin: 10px; background-color: white" align="center"><?php $veh = datosVehiculo($sub_orden['orden_id'], $dbpfx); ?>
					<span><big><b><?php echo $veh['completo'];?></b></big></span>
				</div>
				<div class="col-lg-2"></div>
			</div>

			<div class="row">
				<div class="alert alert-info" role="alert" style="margin: 10px;"><big>Refacciones, Productos, Materiales y Mano de Obra a presupuestar para la Reparación:</big></div><?php
				...
				<div align="center" style="margin: 10px;">
					<table width="100%" style="text-align: center;">
						<tr class="obscuro">
							<td><big><b>Reporte:</b></big></td>
						</tr>
						<tr class="claro">
							<td><?php echo $reporte; ?></td>
						</tr>
						<tr class="obscuro">
							<td><big><b>Área:</b></big></td>
						</tr>
						<tr class="claro">
							<td><?php echo constant('NOMBRE_AREA_' . $sub_orden['sub_area']); ?></td>
						</tr>
						<tr class="obscuro">
							<td><big><b>Descripción de la Tarea:</b></big></td>
						</tr>
						<tr class="claro">
							<td><?php echo $sub_orden['sub_descripcion']; ?></td>
						</tr>
					</table>
				</div>
	...
	echo '<div style="overflow-x:auto; margin: 10px;">';
	...
	$pregsub = "SELECT sub_orden_id, sub_area, fact_id, recibo_id, sub_descuento FROM " . $dbpfx . "subordenes WHERE sub_reporte = '" . $sub_orden['sub_reporte'] . "' AND orden_id = '" . $sub_orden['orden_id'] . "' AND sub_aseguradora = '" . $sub_orden['sub_aseguradora'] . "' AND (sub_orden_id = '" . $sub_orden_id . "' OR sub_area = '";
...
//		echo $preg0 . '<br>';
	...
	echo '<div style="overflow-x:auto; margin: 10px;"><center>';
	...
	echo '			<tr class="cabeza_tabla"><td colspan="2" style="text-align:left;">Seleccionar un Paquete de Servicio ' . $sub_orden['sub_orden_id'] . '</td></tr>
...
// ------ Evitar doble registro del mismo cobro ------
	if($_SESSION['microtime'] != '') {$error = 'si'; $mensaje .= $lang['EnvioDuplicado'].'<br>';}

	if (($error === 'no') && (isset($paquete) || is_array($concepto) || is_array($op_id) )) {
		//--- Al procesar el presupuesto, se coloca a la variable $_SESSION['microtime'] el valor $microtime con lo que...
		//--- si se vuelve a enviar el formulario y existe algún valor en $_SESSION['microtime'], será detectado como doble envío...
...
// ------ Determinamos las tareas en las que se aplicarán los cambios ------
		...
		$tarestat[$sub_orden_id] = $estatus;
		if($aseguradora_id > 0) { $siniestro = 1; } else { $siniestro = 0; }
		$preg = "SELECT sub_orden_id, sub_area, sub_aseguradora, sub_estatus, sub_siniestro, sub_poliza FROM " . $dbpfx . "subordenes WHERE orden_id = '" . $orden_id . "' AND sub_reporte = '" . $reporte . "' AND sub_aseguradora = '" . $aseguradora_id . "' AND ";
		if($oppres == 1) {
...
// ------ Determina el área del concepto para decidir si se agrega a tarea actual ------
			foreach($concepto as $k => $nombre) {
				...
				// Se requiere $ercon = 0; hasta resolver validación de Pintura.
				$ercon = 0;
				...
				if($morep[$k] > 0 && $mocamb[$k] > 0) { $ercon = 1; $msg = $nombre . ' no puede tener MO de reparación y cambio, coloque Monto sólo en una casilla.<br>'; }
				if(($precio[$k] > 0 || $precio[$k] == '#') && $mocamb[$k] <= 0 && $valor['NoVerficMOCamb'][0] != 1) { $ercon = 1; $msg = $nombre . ' requiere mano de obra para cambio.<br>'; }
					...
					for($m = 1; $m < 5; $m++) {
						if($m == 1 && ($precio[$k] > 0 || $precio[$k] == '#')) {
							...
							if($mocamb[$k] > 0) {
								...
								//$op_subtotal2 = round(($cant * $op_precio), 2);
								//$nombre2 = $ccc . ' <= ' . $concepto[$k];
							...
							if($siniestro === 0){
								...
								$npp = (($matpint * 75) / 100);
								$npp2 = (($matpint * 25) / 100);
								$op_subtotal2_CM= $npp2;
								$op_precio_CM = $npp2;
								$cant_CM = 1;
								$nombre_CM = 'Constante de Materiales';
								$tang_CM = 2;
								$areanu_CM = 7;
							}
							$op_subtotal = $npp;
...
// ------ Si no existe una Tarea del área requerida, se crea -------
							...
							$tarea[$areanu] = ejecutar_db($dbpfx . 'subordenes', $sql_data_array, 'insertar');
						}
						if($op_subtotal > 0 || ($precio[$k] == '#' && $m == 1)) {
							...
							if($siniestro === 0){ }
								...
								if($nombre == 'Materiales de Pintura'){
								$sql_data_array30 = array('sub_orden_id' => $tarea[$areanu],
									'op_area' => $areanu_CM,
									'op_item' => $item,
									'op_nombre' => $nombre_CM,
									'op_cantidad' => $cant_CM,
									'op_precio' => $op_precio_CM,
									'op_subtotal' => $op_subtotal_CM,
									'op_autosurtido' => '0',
									'op_pres' => $oppres,
									'op_tangible' => $tang_CM);
								$nueva_id = ejecutar_db($dbpfx . 'orden_productos', $sql_data_array30, 'insertar');
								$item++;
								}
							}
							

							if($qv_activo == 1 && ($oppres != 1 || $preaut == 1) && $m == 1) {
...
//							echo 'Estatus de la tarea: ' . $tarestat[$tarea[$areanu]] . '<br>';
					...
					redirigir('presupuestos.php?accion=' . $hacer . '&sub_orden_id=' . $sub_orden_id . '&area=' . $area);
				}
			}
		}
