...
//	header('Location: https://' . $host . '/' . $url);
	...
	function lista_documento ($orden_id, $estatus, $dbpfx) {
		...
		<tr><td>Nombre</td><td>Archivo</td><td>Fecha de registro</td><td>Usuario</td><td>Vista previa</td>';
			if ($_SESSION['rol06']=='1') {
				echo '<td>Eliminar<br>Descargar</td>';
			} else {
				echo '<td>&nbsp;</td>';
			}
			echo '</tr>';
...
<!--			<td>' . constant('ORDEN_ESTATUS_' . $documento['doc_etapa']) . '</td> -->';
				if ($documento['doc_archivo'] != '') {
					...
					echo 'width="48" border="0"></td>';
				} else {
					echo '			<td><img src="' . DIR_DOCS . 'documento.png" alt="Sin imagen" title="Sin imagen"></td>';
				}
				if ($_SESSION['rol06']=='1' || $_SESSION['rol05']=='1') {
					echo '<td><input type="checkbox" name="eliminar[' . $c . ']" /><input type="hidden" name="doc_id[' . $c . ']" value="' . $documento['doc_id'] . '"/><input type="hidden" name="doc_arch[' . $c . ']" value="' . $documento['doc_archivo'] . '"/><input type="hidden" name="orden_id" value="' . $orden_id . '"/><input type="hidden" name="estatus" value="' . $estatus . '"/></td>';
				} else {
					echo '<td>&nbsp;</td>';
				}
				echo '		</tr>';
...
//		  	$sql_data_array['orden_presupuesto'] = $pres_orden;
   	...
   	if ($orden['orden_ref_pendientes']=='0' && $orden['orden_estatus']>2 && $orden['orden_estatus']< 12 && is_null($orden['orden_fecha_ref_recibidas'])) {
...
//		echo $pregunta3;
	...
	function datosVehiculo($orden_id, $dbpfx) {
		$pregunta = "SELECT v.vehiculo_marca, v.vehiculo_tipo, v.vehiculo_color, v.vehiculo_modelo, v.vehiculo_placas FROM " . $dbpfx . "vehiculos v, " . $dbpfx . "ordenes o WHERE o.orden_vehiculo_id = v.vehiculo_id AND o.orden_id = '" . $orden_id . "'";
