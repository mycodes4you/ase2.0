   		...
   		$orden_id = $orden['orden_id'];
			for($area = 1; $area <= $num_areas_servicio; $area++) {
				...
				while($sub = mysql_fetch_array($matr3)) {
					...
					} elseif($sub['sub_estatus'] < 104 || $sub['sub_estatus'] == 120 || ($sub['sub_estatus'] > 121 && $sub['sub_estatus'] < 130)) {
						$sdoc[] = $sub['sub_estatus'];
...
// 	  		echo 'Calidad -> ' . $calidad . '<br>';
 			for($i = 1; $i <= $num_areas_servicio ; $i++) {
	 			...
	 			} elseif(!is_null($orden['orden_estatus_'.$i]) && ($orden['orden_estatus_'.$i] < 4 || ($orden['orden_estatus_'.$i] > 16 && $orden['orden_estatus_'.$i] < 30 && $orden['orden_estatus_'.$i] != 21))) {
 					$doc[$i] = $orden['orden_estatus_'.$i];
 				}
 				if($orden['orden_estatus_'.$i] == 21 || ($orden['orden_estatus_'.$i] >= 11 && $orden['orden_estatus_'.$i] <= 16)) {
 					$ter[$i] = $orden['orden_estatus_'.$i];
	 			}
 				if($orden['orden_estatus_'.$i] >= 30 && $orden['orden_estatus_'.$i] <= 89) {
...
*/
	   	...
	   	if ($orden['orden_ref_pendientes']=='0' && $sql_data_array['orden_estatus'] > 2 && $sql_data_array['orden_estatus'] < 12 && is_null($orden['orden_fecha_ref_recibidas'])) {
	  		...
	  		if ($sql_data_array['orden_estatus'] != $orden['orden_estatus']) {
	  			$sql_data_array['orden_fecha_ultimo_movimiento'] = date('Y-m-d H:i:s');
		  		$sql_data_array['orden_alerta'] = 0;
			  	bitacora($orden_id, 'Cambio de estatus de ' . $orden['orden_estatus'] . ' a ' . $sql_data_array['orden_estatus'], $dbpfx);
		  	}
