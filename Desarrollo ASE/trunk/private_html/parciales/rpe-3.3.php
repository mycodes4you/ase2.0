...
foreach ($pagos as $kpag => $vpag) {
	$salto = $xml->createTextNode("\n\t\t\t");
	$salto = $xmlpagos->appendChild($salto);
	$xmlpago = $xml->createElement("pago10:Pago");
// --- Agregar atributos del pago ---
	$xmlpago->setAttribute('FechaPago', $vpag['FechaPago']);
	$xmlpago->setAttribute('FormaDePagoP', $vpag['FormaDePagoP']);
	$xmlpago->setAttribute('MonedaP', "MXN");
	$xmlpago->setAttribute('Monto', $vpag['Monto']);
	if($vpag['Banco'] == '') { $vpag['Banco'] = 'Efectivo'; }
	$xmlpago->setAttribute('NomBancoOrdExt', $vpag['Banco']);
	if($vpag['NumOperacion'] == '') { $vpag['NumOperacion'] = 'XX'; }
	$xmlpago->setAttribute('NumOperacion', $vpag['NumOperacion']);
//	$xmlpago->setAttribute('TipoCambioP', "1");
// --- Append del pago ----------
	$xmlpago = $xmlpagos->appendChild($xmlpago);
// --- Crear nodo documento relacionado *************
	foreach($docrel as $kk => $vv) {
		$salto = $xml->createTextNode("\n\t\t\t\t");
...
// --- Agregar atributos del documento relacionado ---
		$xmldocrel->setAttribute('IdDocumento', $vv['IdDocumento']);
		$xmldocrel->setAttribute('Serie', $vv['Serie']);
		$xmldocrel->setAttribute('Folio', $vv['Folio']);
		$xmldocrel->setAttribute('ImpPagado', $vv['ImpPagado']);
		$xmldocrel->setAttribute('ImpSaldoAnt', $vv['ImpSaldoAnt']);
		$xmldocrel->setAttribute('ImpSaldoInsoluto', $vv['ImpSaldoInsoluto']);
		$xmldocrel->setAttribute('MetodoDePagoDR', $vv['MetodoDePagoDR']);
		$xmldocrel->setAttribute('MonedaDR', "MXN");
		$xmldocrel->setAttribute('NumParcialidad', $vv['NumParcialidad']);
...
unset($timbre, $xml, $sobretimbre, $xmltimbre, $paso, $complemento, $t);
	...
	$rpe_id = ejecutar_db($dbpfx . 'facturas', $sql_data, 'insertar');
	unset($sql_data);

	foreach($docrel as $m1 => $d1) {
		...
		$sql_data_array = array('doc_nombre' => $doc_nombre,
			...
			'orden_id' => $d1['doc_ot']);
		ejecutar_db($dbpfx . 'documentos', $sql_data_array, 'insertar');
		bitacora($d1['doc_ot'], $doc_nombre, $dbpfx);
	}

	if(is_numeric($cobro_id) && $cobro_id > 0) {
...
//	echo "<script>window.open('https://" . $host . "/rpe-3.3-pdf.php?axml=" . $nombre_cfdi . ".xml');</script>";
