...
echo '	
    ...
    echo '		<form action="aaa-fiananzas-extendido.php" method="post" enctype="multipart/form-data" name="filtrorep">
...
//		echo '<br>' . $preg2;
		...
		while($gsub = mysql_fetch_array($matr2)) {
            ...
            echo '
                    <td style="padding-left:10px; padding-right:10px;">' . strtoupper($ord['orden_vehiculo_tipo']) . ' ' . strtoupper($ord['orden_vehiculo_color']) . '</td><td style="padding-left:10px; padding-right:10px;">' . $ord['orden_vehiculo_placas'] . '</td><td style="padding-left:10px; padding-right:10px;">' . constant('ORDEN_ESTATUS_' . $ord['orden_estatus']) . '</td>';
...
//			$mirada = '<!-- occ-' . $occ . ' cpm-' . $cpm . ' nppe-' . $nppe . ' cppe-' . $cppe . ' opped-' . $opped . ' -->';
			...
			echo '<td class="area6" style="text-align:right;">$' .  number_format($partes, 2) . '</td>';
			echo '<td class="area6" style="text-align:right;">$' .  number_format($cons, 2) . '</td>';
			echo '<td class="area6" style="text-align:right;">$' .  number_format($mo, 2) . '</td>';
			echo '<td class="area6" style="text-align:right;">$' .  number_format($pres, 2) . '</td>';
			echo '<td class="area6" style="text-align:right; color:' . $asecc . ';">';
...
//					$dedu_fech = $fact['fact_fecha_emision'] . ' ';
			...
			echo '<td class="area6"><a href="entrega.php?accion=cobros&orden_id=' . $ord['orden_id'] . '" target="_blank">' . $fact_num . '</a></td><td class="area6">' . $fact_fech . '</td><td class="area6">' . $cobrada . '</td><td class="area7" style="text-align:right;">$' . number_format($costref, 2) . '</td><td class="area7">$' . number_format($costcons, 2) . '</td><td class="area7">$' . number_format($costtot, 2) . '</td><td class="area7">$' . number_format($costdest, 2) . '</td><td class="area7">' . number_format($totcosts, 2) . '</td><td ';
			if($util < $margutil ) {
