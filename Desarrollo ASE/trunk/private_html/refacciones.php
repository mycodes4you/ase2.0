	...
	<div class="page-content">
		...
		echo '	
				...
				<div class="col-sm-1 padding"> <!-- Export hoja de calculo -->
					<a href="refacciones.php?accion=pendientes&export=1&ordenref=' . $ordenref . '&nombreref=' . $nombreref . '&estadoref=' . $estadoref . '&pedidoref=' . $pedidoref . '&fecharef=' . $fecharef . '">
...
//				echo $preg4;
	...
	if($export == 1){ // ---- Hoja de calculo ---- 
		...
		if(($ordenref != 0 && $precio_venta == '1') || ($solovalacc != '1' && $ordenref != 0 && ($_SESSION['rol05'] == '1' || $_SESSION['rol02'] == '1'))) {
			echo '		
					<tr class="' . $fondo . '">
						<td colspan="5"></td>
						<td>
...
elseif($accion === "act_pre_venta"){
	...
	if($precio_venta == '1' || ($solovalacc != '1' && ($_SESSION['rol05'] == '1' || $_SESSION['rol02'] == '1' ))) {
...
/*					if($prods['op_item_seg'] > 0) {
					...
					} elseif($prods['op_pres'] == 1) {
						$preg7 = "SELECT * FROM " . $dbpfx . "orden_productos WHERE op_item_seg = '" . $prods['op_id'] . "'";
						$matr7 = mysql_query($preg7) or die("ERROR: Fallo selección de orden productos");
						unset($itseguidor);
						while ($itsss = mysql_fetch_array($matr7)) {
							$itseguidor['op_precio'] = $itsss['op_cantidad'] * $itsss['op_precio'];
							$itseguidor['subtotal'] = $itseguidor['subtotal'] + ($itsss['op_precio'] * $itsss['op_cantidad']);
						}
					}*/
								...
								<table border="1" cellpadding="2" cellspacing="0" width="100%">
									<tr>
										<td style="font-weight:bold; text-align:left;">
											<a name="' . $prods['op_id'] . '"></a>' . $prods['op_cantidad'] . ' ' . $prods['op_nombre'];
							if($ajustacodigo == 1) {
								$ref[$tr][$j] .= '<br>Código: <input type="text" name="op_codigo[' . $j . ']" value="' . $prods['op_codigo'] . '" size="12" />';
							}
							$ref[$tr][$j] .= '
											<input type="hidden" name="op_id[' . $j . ']" value="' . $prods['op_id'] . '" />';
							$ref[$tr][$j] .= '
											<input type="hidden" name="prod_id[' . $j . ']" value="' . $prods['prod_id'] . '" />
										</td>';
								...
								function muestraAbajo' . $prods['op_id'] . '() {
    								document.getElementById("AyudaDatosCotizaciones' . $prods['op_id'] . '").classList.toggle("mostrar");
											...
											foreach($opprov[$prods['op_id']] as $miop => $prov3) {
													...
													if((validaAcceso('1050002', $dbpfx) == 1 || ($solovalacc != '1' && ($_SESSION['rol08']=='1'))) && (strtotime($prov3['prod_vencimiento']) > time() || strtotime($prov3['prod_vencimiento']) < 1)) {
											...
											$ref[$tr][$j] .= '							<tr>';
											if(validaAcceso('1050003', $dbpfx) == 1 || ($solovalacc != '1' && ($_SESSION['rol08']=='1'))) {
										...
										if(validaAcceso('1050002', $dbpfx) == 1 || ($solovalacc != '1' && ($_SESSION['rol08']=='1'))) {
										...
										if($ajuprecpres == 1 && $tr == 'pre') { // $ajuprecpres es permiso por usuario 
											if(validaAcceso('1115110', $dbpfx) == 1 || ($revprevent != '1' && $solovalacc != 1 && ($_SESSION['rol02'] == 1 || $_SESSION['rol12'] == 1))) {
											...
											if($grvaux != 1) {
												$ref[$tr][$j] .= '<input type="file" name="fotoref[' . $j . ']" />';
											}
											$ref[$tr][$j] .= '</td>'."\n";
										} else {
...
//						if(($_SESSION['rol02'] ==1 || $_SESSION['rol12'] ==1) && $tr == 'aut') {
						if(validaAcceso('1115110', $dbpfx) == 1 || $grvaux == 1 || ($solovalacc != '1' && ($_SESSION['rol02'] == '1' || $_SESSION['rol08'] == '1' || $_SESSION['rol12'] == '1'))) {
							if($prods['op_item_seg'] != '') {
								...
								if($itseguidor['op_precio'] == 0 && $prods['op_costo'] > 0) {
									$margen = -100;
								} else {
									$margen = round(((($itseguidor['subtotal'] - ($prods['op_costo'] * $prods['op_cantidad'])) / $itseguidor['subtotal']) * 100), 2);
								}
								$item_asociado = 1;
							} else {
							...
							if(validaAcceso('1115110', $dbpfx) == 1 || ($revprevent != '1' && $solovalacc != 1 && ($_SESSION['rol02'] == 1 || $_SESSION['rol12'] == 1))) {
								...
								if($prods['op_precio_revisado'] == '0') {
									...
									if($grvaux == 1) {
										if(validaAcceso('1115110', $dbpfx) == 1 || ($solovalacc != 1 && ($_SESSION['rol02'] == 1 || $_SESSION['rol12'] == 1))) {
											$ref[$tr][$j] .= ' <input type="text" name="precio[' . $j . ']" size="6" />';
										} else{
											$ref[$tr][$j] .= number_format($prods['op_precio'],2);	
										}
...
//		echo '			</table></td></tr>'."\n";
		...
		if(validaAcceso('1115115', $dbpfx) == 1 || ($solovalacc != '1' && ($_SESSION['rol08']=='1' || $_SESSION['rol02']=='1' || $_SESSION['rol12']=='1' || $_SESSION['rol05']=='1' || $_SESSION['rol07']=='1'))) {
			if(validaAcceso('1115115', $dbpfx) == 1 || ($solovalacc != '1' && ($_SESSION['rol02']=='1' || $_SESSION['rol12']=='1'))) {
			...
			if(validaAcceso('1050003', $dbpfx) == 1 || ($solovalacc != '1' && ($_SESSION['rol08']=='1'))) {
				// --- Si esta activo QV, mostrar las opciones de cotizacion
				if($qv_activo == '1') {
					echo '				<option value="12">' . $tipo_cotizacion['12'] . '</option>'."\n";
					echo '				<option value="13">' . $tipo_cotizacion['13'] . '</option>'."\n";
					echo '				<option value="14">' . $tipo_cotizacion['14'] . '</option>'."\n";
				} else {
			...
			if($ajuprecpres == 1 &&  ($solovalacc != '1' && ($_SESSION['rol02']=='1' || $_SESSION['rol12']=='1' || $_SESSION['rol08']=='1'))) {
				echo '				<option value="4"';
				if($_SESSION['rol02']=='1' || $_SESSION['rol12']=='1') { echo ' selected ';} 
				echo '>' . TIPO_PEDIDO_4 . '</option>'."\n";
			}
			if(($solovalacc != '1' && ($_SESSION['rol05']=='1' || $_SESSION['rol07']=='1'))) {
				echo '				<option value="5" selected>' . TIPO_PEDIDO_5 . '</option>'."\n";
			}
			echo '			</select></td></tr>'."\n";
			if(validaAcceso('1050003', $dbpfx) == 1 || validaAcceso('1050002', $dbpfx) == 1 || ($solovalacc != '1' && ($_SESSION['rol08']=='1'))) {
...
//				echo 'Descuento Global a Costo de Compra <input type="text" name="descuento" value="" size="4" /><br>en seleccionadas:<br>';
			} elseif(($solovalacc != '1' && ($_SESSION['rol12']=='1')) && $grupo == 1) {
...
elseif($accion==="gestiona") {
	...
	if ($grvaux == '1' || ($solovalacc != '1' && ($_SESSION['rol08']=='1' || $_SESSION['rol02']=='1' || $_SESSION['rol12']=='1' || $_SESSION['rol13']=='1' || $_SESSION['rol05']=='1' || $_SESSION['rol07']=='1'))) {
...
// ------------------
	...
	if(!is_array($selecionado) && (($tipo_pedido < 4 && $cotizar != '1') || $tipo_pedido == 10 || $tipo_pedido == 11)) {
...
/*					if($valor['EnvSoloQV'][0] == '1') {
						...
						$parmped = "pedido_id = '" . $pedido . "'";
						$sql_array = array('pedido_estatus' => 0);
...
/*				if($refpend == 1) {
				...
				foreach($cotiprod as $w => $x) {
					...
					if($provs[$m[0]]['qvid'] >= 1) { $sql_data['cotqv'] = 1; }
...
<!-- /BODY -->'."\n";
				...
				$cuentaprovs = 1; $cuantosprovs = count($prov_selec);
				unset($llaverej);
				foreach($prov_selec as $k) {
					$m = explode(':', $k);
					$para .= $provs[$m[0]]['email'];
					$semilla = $provs[$m[0]]['qvid'] . $orden_id . $dbpfx;
					$llaverej[$m[0]] = md5($semilla);
					if($cuentaprovs < $cuantosprovs) {
						$para .= ',';
					}
					$cuentaprovs++;
				}
				...
				if($EnvSoloQV == '1' && $qv_activo == 1) {
//print_r($llaverej);
					foreach($llaverej as $km => $vm) {
						$para = $provs[$km]['email'];
						$contenido = '<table class="body-wrap">
	<tr>
		<td></td>
		<td class="container" bgcolor="#F2F2F2">
			<div class="content">
				<a href="https://quien-vende.com/cotizacion-express.php?accion=cotizar&azbcd=' . $vm . '">Abre el formulario de Cotización Express, no necesitas ingresar al sistema Quien-Vende.com</a>
				<br>
				O copia y pega en tu navegador: <pre>https://quien-vende.com/cotizacion-express.php?accion=cotizar&azbcd=' . $vm . '</pre>
				<br>
				Gracias!
			</div>
		</td>
...
elseif($accion==='listar') {
	...
	if(validaAcceso('1115030', $dbpfx) == 1 || ($solovalacc != 1 && ($_SESSION['rol08']=='1'))) {
		// Acceso autotizado
	} else {
		redirigir('usuarios.php?mensaje=Acceso sólo para Almacén, ingresar Usuario y Clave correcta');
	}
...
elseif($accion==='item') {
	...
	if (validaAcceso('1115035', $dbpfx) == 1 || ($solovalacc != 1 && ($_SESSION['rol08']=='1'))) {	
		// Acceso autotizado
	} else {
									...
									<td>
										...
										<select class="form-control" name="uniprod" size="1">
											...
											foreach ($valarr['unidad'] as $k) {
												echo '			
												<option value="'.$k.'" '; if($prod['prod_unidad']==$k) {echo 'selected="selected" ';} echo '>'.$k.'</option>'."\n";
											}
	echo '
										</select>
									</td>
									...
									<td>
										<select class="form-control" name="tangible" size="1">
											...
											foreach ($valarr['prod_tipo'] as $k => $v) {
												echo '
												<option value="' . $k . '" '; if($prod['prod_tangible'] == $k) {echo 'selected="selected" ';} echo '>'. $v .'</option>'."\n";
											}
	echo '		
										</select>
									</td>
	...
	echo '		
								...
								<tr>
									<td align="right">
										<br><br><big><b>Ubicación:</b></big>
									</td>
									<td colspan="2">
										<small>Lugar dentro del almacén en donde está ubicado el producto.</small>
					...
					<div class="form-group">
      					<div class="col-md-12">
							<table>
								...
								<tr>
									<td align="right">
										<br><big><b>Costo de Compra:</b></big>
									</td>
									<td>
										<br><input class="form-control" type="text" name="prodcosto" size="11" maxlength="20" value="'; 
					...
					<div class="form-group">
      					<div class="col-md-12">
							<table>
								<tr>
									<td align="right">
										<big><b>Margen de Utilidad:</b></big></td>
									<td>
										&nbsp;&nbsp;&nbsp;&nbsp;<big><b>' . round(($margen * 100), 2) . '% </b></big>
...
//		echo $preg2 . '<br>';
		...
		$matr3 = mysql_query($preg3) or die("ERROR: Fallo selección de proveedores!");
	  	...
	  	echo '		
			...
			<form action="refacciones.php?accion=cotizacion_pedido" method="post" enctype="multipart/form-data">
			<div class="row">
				<div class="col-md-12">
		...
		if($cotiza > 0) {
			...
			while($cot = mysql_fetch_array($matr3)) {
				echo '
									<input type="hidden" name="pp_id[' . $cot['pp_id'] . ']" value="' . $cot['pp_id'] . '" />
									<input type="hidden" name="prov_id[' . $cot['pp_id'] . ']" value="' . $cot['prod_prov_id'] . '" />
									<input type="hidden" name="producto_id[' . $cot['pp_id'] . ']" value="' . $cot['prod_id'] . '" />
									<tr class="' . $clase . '">
										<td>
	...
	echo '
			...
			<div class="row">
				<div class="col-md-4">		
					<tr>
						<td align="right"><big><b>Tipo de pedido:</b></big></td>
						<td >
...
elseif($accion === 'cotizacion_pedido'){

	echo 'cotizacion_pedido<br>';
	...
	foreach($pp_id as $key => $value){
		...
		// --- Actualizar cotización o cancelarla ---
		if($quitacot[$key] != '1'){
			$parametros = " pp_id = '" . $pp_id[$key] . "' ";
			$sql_data_array = [
				'prod_costo' => limpiarNumero($cot_costo[$key]),
				'dias_entrega' => $cot_entrega[$key],
				'dias_credito' => $cot_credito[$key],
				'prod_cantidad' => $cantidad[$key]
			];
			...
			if($hazped == $key){ // --- Se monta pedido ---
				echo 'Se Monta Pedido<br>';
				if($tipo_pedido == ''){
					$_SESSION['msjerror'] = 'Debes de seleccionar un tipo de pedido.';
					redirigir('refacciones.php?accion=item&prod_id=' . $prod_id);
				} else{ // --- se monta pedido ---
					
					/* ------------ Crear pedidos de almacén --------------------*/
					$proveedor = $prov_id[$key];
					$subtotal = 0; 
					
					$subtotal = ($cantidad[$key] * $cot_costo[$key]);
					...
					$pedido = ejecutar_db($dbpfx . 'pedidos', $sql_array, 'insertar');
					$fpromped = dia_habil($provs[$proveedor]['dde']);
					...
					echo 'prod_id ' . $producto_id[$key] . '<br>';
					
					// --- Consultar info del producto ---
					$preg_prod = "SELECT prod_codigo, prod_nombre, prod_tangible FROM " . $dbpfx . "productos WHERE prod_id = '" . $producto_id[$key] . "'";
					$matr_preg_prod = mysql_query($preg_prod) or die("ERROR: Fallo selección de productos!");
					$info_prod = mysql_fetch_assoc($matr_preg_prod);
					
					echo $preg_prod;
					
					$costoprod = limpiarNumero($cot_costo[$key]);
					$cantprod = limpiarNumero($cantidad[$key]);
					$sql_data = [
						'prod_id' => $producto_id[$key],
						'op_codigo' => $info_prod['prod_codigo'],
						'op_nombre' => $info_prod['prod_nombre'],
						'sub_orden_id' => '999999997',
						'op_pedido' => $pedido,
						'op_cantidad' => $cantprod,
						'op_costo' => $costoprod,
						'op_tangible' => $info_prod['prod_tangible'],
					...
					$preg1 = "UPDATE " . $dbpfx . "productos SET prod_cantidad_pedida = prod_cantidad_pedida + '" . $cantprod . "' WHERE prod_id = '$prod_id[$key]'"."\n";
					...
					$acargo = constant('TIPO_PEDIDO_'.$tipo_pedido);
					$empresa = $provs[$proveedor]['nombre'];
					$contacto = $provs[$proveedor]['contacto'];
					$para = $provs[$proveedor]['email'];
					$enviar_prov = $provs[$proveedor]['env'];
					...
					if($enviar_prov == '1') {
						...
						$mail->SMTPSecure = 'tls';                            // Enable encryption, 'ssl' also accepted
						$mail->Port       = $smtppuerto;
						//$mail->SMTPDebug = 2; // -- 0 Producción 1 Cliente 2 Cliente Servidor
						...
						if($tipo_pedido < '4') {
							$ma = explode(',', $provs[$proveedor]['email']);
							foreach($ma as $k) {
								$mail->addAddress($k);     // Add a recipient
							}
						}
						...
						$cantprod = limpiarNumero($cantidad[$key]);
						$email_order .= '<tr><td>' . $cantidad[$key] . '</td><td>' . $info_prod['prod_nombre'] . '</td><td>' . $info_prod['prod_codigo'] . '</td></tr>'."\n";
			...
			$preg1 = "DELETE FROM " . $dbpfx . "prod_prov WHERE pp_id = '" . $pp_id[$key] . "' ";
...
elseif($accion==="gestprod") {
	...
	foreach($prod_id as $j => $w){
		if($quitar[$j] == '1'){
			$retirar = 1;
			unset($_SESSION['cotped']['prod_id'][$j]);
		}
	}
	if($retirar == 1){
		$_SESSION['msjerror'] = 'Se quitaron los elementos seleccionados.<br>' . $pasar;
...
elseif($accion==="cargacotiza") {
	if(validaAcceso('1115090', $dbpfx) == '1' || ($solovalacc != '1' && ($_SESSION['rol02']=='1' || $_SESSION['rol08']=='1')) || validaAcceso('1050003', $dbpfx) == 1) {
		$msj='Acceso autorizado';
	} else {
		redirigir('usuarios.php?mensaje=Acceso NO autorizado ingresar Usuario y Clave correcta');
	}
...
elseif($accion==="guardacotiza") {
	...
	if (validaAcceso('1115090', $dbpfx) == '1' || validaAcceso('1050003', $dbpfx) == 1 || ($solovalacc != '1' && ($_SESSION['rol02']=='1' || $_SESSION['rol08']=='1'))) {
		$msj='Acceso autorizado';
	} else {
		redirigir('usuarios.php?mensaje=Acceso NO autorizado ingresar Usuario y Clave correcta');
	}
...
elseif($accion==="vercotiza") {
	...
	if (validaAcceso('1115090', $dbpfx) == '1' || ($solovalacc != '1' && ($_SESSION['rol02']=='1' || $_SESSION['rol05']=='1' || $_SESSION['rol08']=='1' || $_SESSION['rol13']=='1'))) {
		$msj='Acceso autorizado';
	} else {
		redirigir('usuarios.php?mensaje=Acceso NO autorizado ingresar Usuario y Clave correcta');
	}
...
elseif($accion==='generacb') {
	...
	if (validaAcceso('1115095', $dbpfx) == '1' || ($solovalacc != '1' && ($_SESSION['rol08']=='1'))) {
		$mensaje='';
	} else {
		 redirigir('usuarios.php?mensaje=Acceso sólo para Almacén, ingresar Usuario y Clave correcta');
	}
