...
//if((defined("L_LANG") && L_LANG != "es_MX" && L_LANG != "L_LANG") || isset($lang) && $lang != "es_MX") include_once("lang/calendar.".(isset($lang) ? $lang : L_LANG).".php");
include_once("lang/calendar.es_MX.php");
...
class tc_calendar{
	...
	function writeScript(){
		...
		if($this->date_picker){
			echo("<div style=\"position: relative; z-index: ".$this->zindex."; float: center;\" id=\"container_".$this->objname."\" onmouseover=\"javascript:focusCalendar('".$this->objname."');\" onmouseout=\"javascript:unFocusCalendar('".$this->objname."', ".$this->zindex.");\">");

			if($this->show_input){
				if($this->hl){
					$to_replace = array("%"," ",".",",","ב","年","日");
					$order = str_replace($to_replace,"",L_CAL_FORMAT);
					if(strpos($order,"Y") == 0 || $this->rtl) $this->writeYear();
					elseif(strpos($order,"B") == 0 && !$this->rtl) $this->writeMonth();
					elseif(strpos($order,"d") == 0 && !$this->rtl) $this->writeDay();					
					if(strpos($order,"Y") == 1 && !$this->rtl) $this->writeYear();
					elseif(strpos($order,"B") == 1 || $this->rtl) $this->writeMonth();
					elseif(strpos($order,"d") == 1 && !$this->rtl) $this->writeDay();					
					if(strpos($order,"Y") == 2 && !$this->rtl) $this->writeYear();
					elseif(strpos($order,"B") == 2 && !$this->rtl) $this->writeMonth();
					elseif(strpos($order,"d") == 2 || $this->rtl) $this->writeDay();					
				}else{
					$this->writeYear();
					$this->writeMonth();
					$this->writeDay();
	...
	function writeYear(){
		// LOS ECHOS COMENTADOS CORRESPONDEN A EL ELEMENTOS SELECT DEFAULT DE EL CALENDARIO, SE AGREGA INPUT CON CSS PARA MOSTRAR LA FECHA SELECCIONADA		
		$url = $_SERVER['PHP_SELF'];
		$busca_esto   = 'ordenes.php';
		$esta_esto = strpos($url, $busca_esto);
		if ($esta_esto === false) {
		echo("<input type=\"textbox\" name=\"".$this->objname."_year\" id=\"".$this->objname."_year\" value=\"$this->year\" maxlength=4 size=3 onBlur=\"javascript:tc_setYear('".$this->objname."', this.value, '$this->path');\" onKeyPress=\"javascript:if(yearEnter(event)){ tc_setYear('".$this->objname."', this.value, '$this->path'); return false; }\" class=\"css-input\" disabled style=\"width: 35px;\"> ");
		} else {
		  //echo 'desactivado';
		}
 ...
 ?></div><?php 
	...
	function writeMonth(){
		...
		$esta_esto = strpos($url, $busca_esto);
		if ($esta_esto === false) {
			echo("<input type=\"textbox\" name=\"".$this->objname."_month\" id=\"".$this->objname."_month\" value=\"$this->month\" maxlength=4 size=3 onBlur=\"javascript:tc_setYMonth('".$this->objname."', this.value, '$this->path');\" onKeyPress=\"javascript:if(monthEnter(event)){ tc_setMonth('".$this->objname."', this.value, '$this->path'); return false; }\" class=\"css-input\" disabled> ");
		} else {
		  //echo 'desactivado';
		}
	...
	function writeDay(){
		...
		$esta_esto = strpos($url, $busca_esto);
		if ($esta_esto === false) {
		  echo("<input type=\"textbox\" name=\"".$this->objname."_day\" id=\"".$this->objname."_day\" value=\"$this->day\" maxlength=4 size=3 onBlur=\"javascript:tc_setYDay('".$this->objname."', this.value, '$this->path');\" onKeyPress=\"javascript:if(dayEnter(event)){ tc_setDay('".$this->objname."', this.value, '$this->path'); return false; }\" class=\"css-input\" disabled> ");
		} else {
		  //echo 'desactivado';
		}
