...
//		$fincot = end($fincot);
		...
		echo '</td></tr>'."\n";
		echo '			<tr><td>Inicio de Presupuesto: ' . date('Y-m-d H:i', strtotime($inipres[0]['fecha'])) . '</td>';
		echo '<td>Presupuesto por: ' . $usr[$inipres[0]['usuario']]['nombre'] . '</td><td>OT: ' . $orden_id . '</td></tr>'."\n";
//		echo '			<tr><td colspan="3">Presupuesto terminaldo el : ' . date('Y-m-d H:i', strtotime($inipres[0]['fecha'])) . '</td></tr>'."\n";
		echo '			<tr><td colspan="3">Presupuesto terminaldo el : ' . print_r($finpres) . '</td></tr>'."\n";
		echo '			<tr><td colspan="3">Propiedad de : ' . $emp['empresa_razon_social'] . '</td></tr>'."\n";
		echo '			<tr><td colspan="3"><span><strong>Unidad:' . $veh['completo'] . '</strong></span></td></tr>'."\n";
		echo '		</table>'."\n";
		$diasrep = 1;
