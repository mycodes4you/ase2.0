...
if($dato['cliente_telegram_id'] != '' ) {
	...
	$texto1 = 'Estimado(a) ' . $dato['cliente_nombre'] . ' ' . $dato['cliente_apellidos'] . '. Le informamos que la Aseguradora ' . constant('ASEGURADORA_NIC_'.$aseguradora) . ' ha autorizado la reparación de su vehículo con relación al siniestro: ' . $reporte . ' que atendemos con la Orden de Trabajo ' . $orden_id . ' para el vehículo Marca: ' . $dato['orden_vehiculo_marca'] . ' Tipo: ' . $dato['orden_vehiculo_tipo'] . ' Color: ' . $dato['orden_vehiculo_color'] . ' Placas: ' . $dato['orden_vehiculo_placas'] . '.';

	if($dedu > 0) {
		$texto2 = ' Por otra parte, le comunicamos que su aseguradora determinó que tiene un deducible por pagar para el siniestro: '. $reporte . '; el monto y la cuenta en donde tendrá que depositarlo le será informado por su asesor de servicio. Le recordamos que nosotros por ningún motivo aceptamos efectivo, por lo que le solicitamos presente el voucher original del pago al momento de recoger el vehículo. Gracias.';
	}

	$texto3 = ' Atentamente: ' . $usu0['nombre'] . ' ' . $usu0['apellidos'];

	$motivo = $asunto . ' ' . $texto1 . ' ' . $texto2 . ' ' . $texto3;
        $notitelegram = notificaTelegram($dato['cliente_telegram_id'], $motivo);
//      echo $notitelegram;
} else {

   $para = $agencia_email;
   $asunto = 'Cliente sin Telegram ID registrado.' ;
