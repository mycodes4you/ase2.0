...
// --- Parámetros iniciales ---
	$tarifa1 = 100; $tarifa2 = 120; $fijo = 1000;
	$feinicalc = '2015-01-01 00:00:00';

// --- Se crea la pregunta de usuarios con rol de asesor ---
$pregase = "SELECT usuario, nombre, apellidos FROM " . $dbpfx . "usuarios WHERE acceso = '0' AND codigo = '20' ORDER BY nombre, apellidos ";
...
// --- Obtener OTs 99 del periodo y localizar si tienen comisiones calculadas --
	$preg1 = "SELECT " . $dbpfx . "ordenes.orden_id, comision_tipo, recibo_id FROM " . $dbpfx . "ordenes LEFT JOIN " . $dbpfx . "comisiones ON " . $dbpfx . "ordenes.orden_id = " . $dbpfx . "comisiones.orden_id WHERE orden_fecha_de_entrega < '" . $fefin . "' AND orden_fecha_de_entrega > '" . $feini . "' AND orden_servicio = '4' AND orden_estatus = '99'";
	$matr1 = mysql_query($preg1) or die("ERROR: Fallo selección de OTs! tipo-3 67 " . $preg1);
//	echo $preg1;
//	echo mysql_num_rows($matr1);
	...
	while ($ots = mysql_fetch_array($matr1)) {
		...
		while ($ases = mysql_fetch_array($matrase)) {
			$preg3 = "SELECT usuario FROM " . $dbpfx . "bitacora WHERE orden_id = '" . $ots['orden_id'] . "' AND `bit_estatus` LIKE 'Cambio a estatus 4 %anterior: 2%' LIMIT 1";
			$matr3 = mysql_query($preg3) or die("ERROR: Fallo selección de OTs! tipo-3 78 " . $preg3);
			while ($bit = mysql_fetch_array($matr3)) {
//				echo 'OT: ' . $ots['orden_id'] . ' Usuario Rol 5: ' . $ases['usuario'] . ' Usuario que hizo el cambio: ' . $bit['usuario'] . '<br>';
				if($ases['usuario'] == $bit['usuario'] && $ots['comision_tipo'] != $comision) {
					$asesor[$ases['usuario']]['ot'][] = $ots['orden_id'];
					$asesor[$ases['usuario']]['cuantas']++;
					$asesor[$ases['usuario']]['nombre'] = $ases['nombre'] . ' ' . $ases['apellidos'];
				} elseif($ases['usuario'] == $bit['usuario'] && $ots['comision_tipo'] == $comision) {
...
// --- Localizar posibles pagos de comisiones base fijas ---
	...
	$matr2 = mysql_query($preg2) or die("ERROR: Fallo selección de OTs! tipo-1 91 " . $preg2);
//	echo $preg2 . '<br>';
	mysql_data_seek($matrase,0);
	while ($ases = mysql_fetch_array($matrase)) {
