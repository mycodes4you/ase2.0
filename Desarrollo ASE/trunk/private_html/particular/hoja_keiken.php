...
//			echo '		<td style="width:277px; vertical-align: top; padding-right:3px; font-size:10px;">'."\n";
			...
			echo '				<tr><td>Delantera Izquierda</td><td>' . $inv['inv_lizq_lld'] . '</td></tr>'."\n";
			echo '				<tr><td>Delantera Derecha</td><td>' . $inv['inv_lder_lld'] . '</td></tr>'."\n";
			echo '				<tr><td>Trasera Izquierda</td><td>' . $inv['inv_lizq_llt'] . '</td></tr>'."\n";
			echo '				<tr><td>Trasera Derecha</td><td>' . $inv['inv_lder_llt'] . '</td></tr>'."\n";
			...
			echo '			</table>'."\n";
			echo '			<table cellpadding="1" cellspacing="0" border="0" width="100%" class="diagnostico">'."\n";
			echo '				<tr><td colspan="2" rowspan="3" style="width:68%;">¿El consumidor suministra las partes, refacciones o materiales para la prestación del servicio de reparación del vehículo?:<br>Sí( ) No( )</td><td style="text-align:right; width:11%;">Subtotal:</td><td class="cierradiag" style="width:21%;"></td></tr>'."\n";
			echo '				<tr><td style="text-align:right; width:11%;">IVA:</td><td class="cierradiag" style="width:21%;"></td></tr>'."\n";
			echo '				<tr><td style="text-align:right; width:11%;">Total:</td><td class="cierradiag" style="width:21%;"></td></tr>'."\n";
			echo '			</table>'."\n";
			...
			echo '				<tr><td style="border-style:solid; border-width:1px; vertical-align:top; height:145px; line-height:12px;">Observaciones y comentarios: ';
			...
			if($deschi == '1') {
				$preg = "SELECT sub_area, sub_descripcion FROM " . $dbpfx . "subordenes WHERE orden_id = '$orden_id' AND (sub_area = '1' || sub_area = '6' || sub_area = '7' ) AND sub_estatus < '190'";
				$matr = mysql_query($preg) or die("ERROR: Fallo selección de tareas!");
				$tareas = array();
				while($am = mysql_fetch_array($matr)) {
					$tareas[$am['sub_area']] .= $am['sub_descripcion'] . ' '; 
				}
				echo '<br>';
				if($tareas[6]!='') { echo NOMBRE_AREA_6 . ': ' .$tareas[6] .'<br>';} 
				if($tareas[7]!='') { echo NOMBRE_AREA_7 . ': ' .$tareas[7] .'<br>';} 
				if($tareas[1]!='') { echo NOMBRE_AREA_1 . ': ' .$tareas[1];} 
			}
			echo '				</td></tr>
			</table>'."\n";
			echo '		</td></tr>'."\n";
