...
//			$vehiculo = datosVehiculo($orden_id, $dbpfx);
		...
		echo '				<tr><td style="text-align:left; font-size:22px; font-weight:bold; line-height:30px;">' . $vehiculo['marca'] . ' ' . $vehiculo['tipo'] . ' ' . $vehiculo['color'] . ' ' . $vehiculo['modelo'] . $lang['Placas'] . $vehiculo['placas'] . '<br>Orden de Trabajo: ' . $orden_id . '</td><td style="text-align:right;">';
		echo '</td></tr>'."\n";
		echo '			</table>'."\n";
...
//				$codigo = $orden_id . ' ' . $sub['sub_orden_id'] . ' ' . $sub['sub_operador'];
	      ...
	      $matriz3 = mysql_query($pregunta3) or die("ERROR: Fallo seleccion!");
			...
			echo '				<tr><td style="text-align:left; font-size:22px; line-height:30px; width:748px;"><b><strong>Tarea ' . $sub['sub_orden_id'] . ': ' . constant('NOMBRE_AREA_' . strtoupper($sub['sub_area'])) . ' ' . $sub['sub_descripcion'] . '</strong></b><br><b><small>CONVENIO:</b> ' . $nick_convenio['aseguradora_nic'] . ' <b>SINIESTRO:</b> ' . $sub['sub_reporte'] . '</small></td><td valign="top" style="width:100px;" ><img src="' . $imagenseg . '" alt="Código QR registro de seguimiento de reparación" width="100"></td></tr></table>'."\n";
