...
// Company
function addSociete( $nom, $adresse )  // Datos de Emisor
{
	$x1 = 10;
	$y1 = 7;
...
// Invoice
function addFacture( $numfact ) // Factura
{
	$string = sprintf("FA%04d",$numfact);
	$this->fact_dev( "Facture", $string );
}

function addDate( $date ) // Fecha de emisión
{
	$r1  = $this->w - 51;
	$r2  = $r1 + 40;
	$y1  = 17;
...
function addClient( $ref ) // Número de cliente
{
	$r1  = $this->w - 31;
	$r2  = $r1 + 19;
	$y1  = 17;
...
function addClientAdresse( $adresse )
{
	...
	$this->SetXY( $r1, $y1);
	$this->SetFont( "Arial", "", 10);
	$this->MultiCell( 170, 4, $adresse);
}

// Mode of payment
function addReglement( $mode ) // Lugar de Emisión
{
	$r1  = 10;
	$r2  = $r1 + 60;
	$y1  = 80;
	...
	$this->SetFont( "Arial", "B", 10);
	$this->Cell(10,4, "Lugar de Emisión", 0, 0, "C");
...
// Expiry date
function addEcheance( $date ) // No de Serie del CSD
{
	$r1  = 80;
	$r2  = $r1 + 40;
	$y1  = 80;
	...
	$this->SetXY( $r1 + ($r2 - $r1)/2 - 5 , $y1+1 );
	$this->SetFont( "Arial", "B", 10);
	$this->Cell(10,4, "No de Serie del CSD", 0, 0, "C");
	$this->SetXY( $r1 + ($r2-$r1)/2 - 5 , $y1 + 5 );
	$this->SetFont( "Arial", "", 9);
	$this->Cell(10,5,$date, 0,0, "C");
}
...
// VAT number
function addNumTVA($tva)  // Folio Fiscal
{
	$this->SetFont( "Arial", "B", 10);
	$r1  = $this->w - 80;
	$r2  = $r1 + 70;
	...
	$this->Cell(38, 4, "Folio Fiscal", '', '', "C");
	$this->SetFont( "Arial", "", 9);
	$this->SetXY( $r1 + 16 , $y1+5 );
	$this->Cell(38, 5, $tva, '', '', "C");
}

function addReference($ref) // Referencia
{
	$this->SetFont( "Arial", "", 10);
...
function addConletra($letra, $pos)
{
	$this->SetFont( "Arial", "", 8);
	$length = $this->GetStringWidth( "Total con letra: " . $letra );
	$r1  = 10;
	$r2  = $r1 + $length;
	$y1  = $pos;
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->Cell($length,4, "Total con letra: " . $letra);
}

function addFormaPago($motivo, $pos)
{
	$this->SetFont( "Arial", "", 8);
	$length = $this->GetStringWidth( "Forma de Pago: " . $motivo );
	$r1  = 10;
	$r2  = $r1 + $length;
	$y1  = $pos;
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->Cell($length,4, "Forma de Pago: " . $motivo);
}

function addMetodoPago($motivo, $pos)
{
	$this->SetFont( "Arial", "", 8);
	$length = $this->GetStringWidth( "Método de Pago: " . $motivo );
	$r1  = 10;
	$r2  = $r1 + $length;
	$y1  = $pos;
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->Cell($length,4, utf8_decode("Método de Pago: ") . $motivo);
}

function addCuentaPago($motivo="No Identificada", $pos)
{
	$this->SetFont( "Arial", "", 8);
	$length = $this->GetStringWidth( "Número de Cuenta de Pago: " . $motivo );
	$r1  = 10;
	$r2  = $r1 + $length;
	$y1  = $pos;
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->Cell($length,4, "Número de Cuenta de Pago: " . $motivo);
}

function addCondicionesPago($motivo=null, $pos)
{
	$this->SetFont( "Arial", "", 8);
	$length = $this->GetStringWidth( "Condiciones de Pago: " . $motivo );
	$r1  = 10;
	$r2  = $r1 + $length;
	$y1  = $pos;
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->Cell($length,4, "Condiciones de Pago: " . $motivo);
}

function addMotivo($motivo, $pos)
{
	$this->SetFont( "Arial", "", 8);
	$length = $this->GetStringWidth( "Motivo de descuento: " . $motivo );
	$r1  = 10;
	$r2  = $r1 + $length;
	$y1  = $pos;
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->Cell($length,4, "Motivo de descuento: " . $motivo);
}

function addVehDatos($datos, $pos)
{
	...
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->MultiCell($length,4, $datos);
}

function addSello($sello, $pos)
{
	$this->SetFont( "Arial", "", 7);
	$r1  = 10;
	$r2  = $this->w - ($r1 * 2) ;
	...
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->MultiCell(190,4, "Sello digital del CFDI: \n" . $sello);
}

function addSelloSAT($sello, $pos)
{
	$this->SetFont( "Arial", "", 7);
	$r1  = 10;
	$r2  = $this->w - ($r1 * 2) ;
	...
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->MultiCell(190,4, "Sello del SAT: \n" . $sello);
}

function addCadenaSAT($version, $uuid, $fecha, $rfcpac, $sello, $num, $pos)
{
	$this->SetFont( "Arial", "", 7);
	$r1  = 10;
	$r2  = $this->w - ($r1 * 2) ;
	...
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$cadena = '||' . $version . '|' . $uuid . '|' . $fecha . '|' . $sello . '|' . $num . '||';
	$this->MultiCell(190,4, utf8_decode("Cadena Original del complemento de certificación digital del SAT: \n") . $cadena);
	$this->SetFont( "Arial", "", 8);
	$this->SetXY( $r1 , $y1+22 );
	$this->Cell(190,4, utf8_decode("No Certificado SAT: " . $num . " Fecha Timbrado: " . $fecha . " RFCProvCertif: " . $rfcpac), 0, 0, "C");
	$this->SetFont( "Arial", "", 10);
	$this->SetXY( $r1 , $y1+30 );
	$this->Cell(190,4, utf8_decode("Este documento es una representación impresa de un CFDI"), 0, 0, "C");
}
...
function addTotales($subtotal="0.00", $descuento="0.00", $nuevosub="0.00", $iva="0.00", $total="0.00", $pos)
{
	$r1  = $this->w - 65;
	$r2  = $r1 + 55;
	$y1  = $pos;
...
//	$this->SetFont( "Arial", "B", 10);
	...
	$this->Cell(30,4, "SubTotal", 0, 0, "R");
	$this->SetXY( $r1+35, $y1+2 );
	$this->Cell(20,4, "$".$subtotal, 0, 0, "R");
	$this->SetXY( $r1-5, $y1+7 );
	$this->Cell(30,4, "Descuento", 0, 0, "R");
	$this->SetXY( $r1+35, $y1+7 );
	$this->Cell(20,4, "$".$descuento, 0, 0, "R");
	$this->SetXY( $r1-5, $y1+12 );
	$this->Cell(30,4, "SubTotal", 0, 0, "R");
	$this->SetXY( $r1+35, $y1+12 );
	$this->Cell(20,4, "$".$nuevosub, 0, 0, "R");
	$this->SetXY( $r1-5, $y1+17 );
	$this->Cell(30,4, "IVA al 16%", 0, 0, "R");
	$this->SetXY( $r1+35, $y1+17 );
	$this->Cell(20,4, "$".$iva, 0, 0, "R");
	$this->SetXY( $r1-5, $y1+22 );
	$this->Cell(30,4, "Total", 0, 0, "R");
	$this->SetXY( $r1+35, $y1+22 );
