...
function addMetodoPago($motivo, $pos)
{
	...
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->Cell($length,4, "Método de Pago: " . $motivo);
}
...
function addCadenaSAT($version, $uuid, $fecha, $sello, $num, $pos)
{
	$this->SetFont( "Arial", "", 7);
	$r1  = 10;
	$r2  = $this->w - ($r1 * 2) ;
	...
	$this->MultiCell(190,4, "Cadena Original del complemento de certificación digital del SAT: \n" . $cadena);
	$this->SetFont( "Arial", "", 10);
	$this->SetXY( $r1 , $y1+22 );
	$this->Cell(190,4, "No de Serie del Certificado del SAT: " . $num . " Fecha y hora de certificación: " . $fecha);
	$this->SetXY( $r1 , $y1+26 );
 	$this->Cell(190,4, "Este documento es una representación impresa de un CFDI", 0, 0, "C");
}
