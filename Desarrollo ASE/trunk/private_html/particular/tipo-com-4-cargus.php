...
// --- Obtener periodo de cálculo ---
	...
	if($feini == '') {
		$feini = date('Y-m-01 00:00:00', strtotime($fefin));
	}
...
// --- Presentar selector de periodo ---
	...
	echo '				<div class="row"><div class="col-sm-3 izq"><strong><big>Fecha de Inicio</big></strong><br>';
		...
		$myCalendar->setYearInterval(2018, 2023);
...
// --- Ajustar horas de fechas seleccionadas --
	...
	$preg3 = "SELECT f.fact_id, f.orden_id, f.fact_tipo, f.fact_num, f.fact_monto, f.fact_fecha_cobrada FROM " . $dbpfx . "facturas_por_cobrar f, " . $dbpfx . "ordenes o WHERE o.orden_fecha_de_entrega < '" . $fefin . "' AND o.orden_fecha_de_entrega > '" . $feini . "' AND f.aseguradora_id = '0' AND f.fact_cobrada = '1' AND f.orden_id = o.orden_id AND f.fact_tipo < 3 ORDER BY aseguradora_id,fact_id ";
	$matr3 = mysql_query($preg3) or die("ERROR: Fallo selección de Facturas cobradas a particulares o de venta adicional! tipo-3 66 " . $preg3);
//		echo $preg3;
	$idx = 0;
...
//				echo 'Asesor: ' . $ases['usuario'] . ' ASE OT: ' . $ots['orden_asesor_id'] . '<br>';
				if($ases['usuario'] == $ots['orden_asesor_id'] && $ots['comision_tipo'] != $comision) {
					...
					$asesor[$ases['usuario']][$idx]['tipo'] = $fact['fact_tipo'];
...
// --- Presentar resultados para seleccionar a quienes se paga ---
	...
	foreach($asesor as $kf => $xf) {
		...
		foreach($xf as $kx => $vf) {
			if(is_numeric($vf['ot'])) {
				if($vf['tipo'] == 1) {
					$montsinimp = round(($vf['monto'] / (1 + $impuesto_iva)),2);
				} else {
					$montsinimp = $vf['monto'];
				}
				$montcom = round(($montsinimp * $tarifa1),2);
				echo '							<tr class="' . $fondo . '">
								<td><input type="checkbox" name="otcom[' . $vf['ot'] . ']" checked value="' . $kf . '|' . $vf['fact_id'] . '|' . $montcom . '|' . $vf['numero'] . '|' . $montsinimp . '|' . $vf['tipo'] . '" /><a href="entrega.php?accion=cobros&orden_id=' . $vf['ot'] . '" target="_blank">' . $vf['ot'] . '</a></td>
								<td>' . $vf['numero'] . '</td>
...
// --- Agregar comisiones para cada Asesor ---
	foreach($item as $k => $v) {
		if($v['tipo'] == 1) { $doctipo = 'Factura'; } else { $doctipo = 'Remisión'; }
		...
		$sql_com = [
			...
			'orden_id' => $k,
			'indicador' => 'Venta de $' . number_format($v['monto'], 2) .  ' en ' . $doctipo . ' ' . $v['numero'],
			'usuario' => $v['asesor'],
			'monto' => $v['montcom'],
...
// --- Inserta el regitro en el recibo de destajo ---
		...
		$descripcom = utf8_encode($com['com_nombre']) . '. Venta: $' . number_format($v['monto'], 2);
		$sql_data = [
				'recibo_id' => $recibo_id[$v['asesor']],
				'orden_id' => $k,
				'monto' => $v['montcom'],
				'reporte' =>  $doctipo . ': ' . $v['numero'],
