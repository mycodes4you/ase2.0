...
if (($accion==='insertar') || ($accion==='validar') || ($accion==='asignar') || ($accion==='seguro') || ($accion==='express') || ($accion==='seguro')) {
...
//	echo 'placas sess: ' . $_SESSION['exp']['placas'] . '<br>';
//	echo 'Empresa: ' . $empresa_id . '<br>';	
	...
	if($filas == 1) {
		...
		$cliente_id = $veh['vehiculo_cliente_id'];
		$existe = 1;
		unset($_SESSION['exp']);

		$preg1 = "SELECT * FROM " . $dbpfx . "clientes WHERE cliente_id = '" . $cliente_id . "'";
//		echo $preg1;
		$matr1 = mysql_query($preg1) or die("ERROR: Fallo selección de Cliente! ".$preg1);
		$fila1 = mysql_num_rows($matr1);
		$clie = mysql_fetch_array($matr1);

		$preg2 = "SELECT * FROM " . $dbpfx . "empresas WHERE empresa_id = '" . $clie['cliente_empresa_id'] . "'";
//		echo $preg2;
		$matr2 = mysql_query($preg2) or die("ERROR: Fallo selección de empresas! ".$preg2);
		$empresa = mysql_fetch_array($matr2);
		$_SESSION['exp']['razon'] = $empresa['empresa_razon_social'];
		if($fila1 < 2) {
			$cliente_id = $clie['cliente_id'];
			$_SESSION['exp']['nombre'] = $clie['cliente_nombre'];
			$_SESSION['exp']['apellidos'] = $clie['cliente_apellidos'];
			$_SESSION['exp']['tipo'] = $clie['cliente_tipo'];
			$_SESSION['exp']['email'] = $clie['cliente_email'];
			$_SESSION['exp']['telefono1'] = $clie['cliente_telefono1'];
			$_SESSION['exp']['telefono2'] = $clie['cliente_telefono2'];
			$_SESSION['exp']['movil'] = $clie['cliente_movil'];
			$_SESSION['exp']['movil2'] = $clie['cliente_movil2'];
		} else {
			$placas = $_SESSION['exp']['placas'];
			$razon = $_SESSION['exp']['razon'];
			redirigir('reg-express.php?accion=validar&empresa_id=' . $empresa_id);
		}
	} elseif($filas > 1) {
		...
		$_SESSION['msjerror'] = 'Buscar si el Cliente ya está Dado de Alta o crear uno nuevo y también agregar al cliente su vehículo.';
		redirigir('personas.php?accion=crear');
	}
	...
	echo '							<h3>' . $lang['Contacto'] . '</h3>'."\n";
	echo '									<table cellpadding="0" cellspacing="0" border="0" class="izquierda" width="100%">'."\n";
	echo '									<tr><td>' . $lang['Nombre'] . '</td><td>' . $clie['cliente_nombre'] . '</td></tr>'."\n"; 
	echo '									<tr><td>' . $lang['Apellidos'] . '</td><td>' . $clie['cliente_apellidos'] . '</td></tr>'."\n";
	echo '									<tr><td>' . $lang['Empresa'] . '</td><td>' . $empresa['empresa_razon_social'] . '</td></tr>'."\n";
	echo '									<tr><td colspan="2"><a href="personas.php?accion=modificar&cliente_id=' . $cliente_id . '"><button>Modificar Cliente</button></a></td></tr>'."\n";
	echo '								</table>'."\n";
	echo '						</div>'."\n";

	echo '						<div class="obscuro espacio">
							<h3>Datos Complementarios de Contacto</h3>
								<table cellpadding="0" cellspacing="0" border="0" class="izquierda" width="100%">'."\n";
	if($existe == 1) {
		echo '									<tr><td>' . $lang['conductor'] . '</td><td colspan="3">';
		if($clie['cliente_tipo'] == 1) { echo $lang['clietipo']; } else { echo $lang['tercero']; }
		echo '</td></tr>'."\n";
		echo '									<tr><td>' . $lang['email'] . '</td><td colspan="3">' . $clie['cliente_email'] . '</td></tr>'."\n";
		echo '									<tr><td>' . $lang['Teléfono'] . '</td><td>' . $clie['cliente_telefono1'] . '</td><td>' . $lang['Otro'] . '</td><td>' . $clie['cliente_telefono2'] . '</td></tr>'."\n";
		echo '									<tr><td>' . $lang['Celular'] . '</td><td>' . $clie['cliente_movil'] . '</td><td>' . $lang['Nextel'] . '</td><td>' . $clie['cliente_movil2'] . '</td></tr>'."\n";
	} else {
		echo '									<tr><td>' . $lang['conductor'] . '</td><td colspan="3">' . $lang['clietipo'] . '<input type="radio" name="clietipo" value="1"';
		...
		echo ' /></td></tr>'."\n";
		echo '									<tr><td>' . $lang['email'] . '</td><td colspan="3"><input type="text" name="email" size="24" maxlength="120" value="' . $_SESSION['exp']['email'] . '" /></td></tr>'."\n";
		echo '									<tr><td>' . $lang['Teléfono'] . '</td><td><input type="text" name="telefono1" size="10" maxlength="40" value="' . $_SESSION['exp']['telefono1'] . '" /></td><td>' . $lang['Otro'] . '</td><td><input type="text" name="telefono2" size="10" maxlength="40" value="' . $_SESSION['exp']['telefono2'] . '" /></td></tr>'."\n";
		echo '									<tr><td>' . $lang['Celular'] . '</td><td><input type="text" name="movil" size="10" maxlength="40" value="' . $_SESSION['exp']['movil'] . '" /></td><td>' . $lang['Nextel'] . '</td><td><input type="text" name="movil2" size="10" maxlength="40" value="' . $_SESSION['exp']['movil2'] . '" /></td></tr>'."\n";
	}
	echo '								</table>'."\n";
	echo '						</div>'."\n";
	echo '					</td>'."\n";
	...
	if($existe != 1) {
		echo '							<form action="reg-express.php?accion=validar" name="empclie" method="post" enctype="multipart/form-data">'."\n";
	}
	echo '							<table cellpadding="0" cellspacing="0" border="0" class="izquierda';
	if($existe == 1) { echo ' consulta'; }
	echo '" width="100%">'."\n";
	...
	if($existe == 1) {
		...
		if($placas != '') {
			$_SESSION['exp']['placas'] = $placas;
		} else {
			$_SESSION['exp']['placas'] = $ord['orden_vehiculo_placas'];
		}
		echo '<input type="hidden" name="siexiste" value="0" />';
		echo '<input type="text" name="placas" size="10" maxlength="15" value="' . $_SESSION['exp']['placas'] . '" onchange="document.empclie.submit()" />';
	}
	echo '<input type="hidden" name="orden_id" value="' . $orden_id . '" /><input type="hidden" name="oid" value="' . $oid . '" />'."\n";
	echo '									</td></tr>'."\n";
	echo '								<tr><td>' . $lang['serie'] . '</td><td colspan="2">';
	if($existe == 1) {
		echo $veh['vehiculo_serie'];
	} else {
		echo '<input type="text" name="serie" size="10" maxlength="30" value="' . $_SESSION['exp']['serie'] . '" />';
	}
	echo '									</td></tr>'."\n";
	...
	echo '									</td></tr>'."\n";
	echo '									<tr><td>' . $lang['tipo'] . '</td><td colspan="2">';
	if($existe == 1) {
	...
	echo '									</td></tr>'."\n";
	echo '									<tr><td>' . $lang['subtipo'] . '</td><td colspan="2">';
	if($existe == 1) {
	...
	echo '									</td></tr>'."\n";
	echo '									<tr><td style="text-align:center;">' . $lang['year'] . '</td><td style="text-align:center;">' . $lang['puertas'] . '</td><td  style="text-align:center;">' . $lang['color'] . '</td></tr>'."\n";
	if($existe == 1) {
	...
	if($existe == 1) {
		echo '						<a href="vehiculos.php?accion=modificar&vehiculo_id=' . $vehiculo_id . '"><button>Modificar</button></a>'."\n";
	} else {
		echo '						<button name="validar" value="1" type="submit">Validar</button>'."\n";
		echo '						</form>'."\n";
	}
...
*/	
	...
	echo '							<h3>' . $lang['Tipo de Servicio'] . '</h3>
								<table cellpadding="0" cellspacing="0" border="0" class="izquierda" width="100%">'."\n";
	if($confolio == 1) {
		echo '									<tr><td colspan="2">Orden de Trabajo: <input type="text" name="ordenid" size="8" maxlength="8" value="' . $orden_id . '" /></td></tr>'."\n";
	}
	...
	echo '									<tr><td colspan="2">' . $lang['docadmin'] . '<input type="file" name="orden_adm" size="6" /></td></tr>'."\n";
	echo '									<tr><td colspan="2">' . $lang['docrep'] . '<input type="file" name="levante" size="6" /></td></tr>'."\n";
	echo '								</table>'."\n";
	echo '						</div>'."\n";
...
//	echo '					<option value="" >Seleccionar... </option>' . "\n";
	...
	echo '									<tr><td>' . $lang['Grua'] . '</td><td style="text-align:left;">Sí<input type="radio" name="grua" value="1" ';
...
/*	if($placas != $_SESSION['exp']['placas'] && $placas != '') {
		$_SESSION['exp']['placas'] = strtoupper(limpiarString($placas));
		redirigir('reg-express.php?accion=express&orden_id=' . $orden_id . '&oid=' . $oid . '&placas=' . $placas);
	}
*/
...
//	echo 'placas sess: ' . $_SESSION['exp']['placas'] . '<br>';	
	...
	if (($nombre!='') || ($apellidos!='')) {
		...
		if ($nombre) {$preg0 .= "cliente_nombre LIKE '%$nombre%' ";}
		if (($nombre) && ($apellidos)) {$preg0 .= "AND cliente_apellidos LIKE '%$apellidos%' ";}
		elseif ($apellidos) {$preg0 .= "cliente_apellidos LIKE '%$apellidos%' ";}
		...
		if($fila0 > 0) {
			while ($clie = mysql_fetch_array($matr0)) {
				$preg1 = "SELECT empresa_razon_social FROM " . $dbpfx . "empresas WHERE empresa_id = '" . $clie['cliente_empresa_id'] . "'";
				$matr1 = mysql_query($preg1) or die("ERROR: Fallo selección de clientes! " . $preg1);
				$emp = mysql_fetch_array($matr1);
				$emprs[$clie['cliente_id']] = array($clie['cliente_empresa_id'], $emp['empresa_razon_social'], $emp['empresa_rfc'], $clie['cliente_nombre'], $clie['cliente_apellidos'], $clie['cliente_telefono1'], $clie['cliente_email']);
			}
		}
	} elseif ($razon!='') {
		$error = 'no'; $mensaje ='';
		$preg1 = "SELECT empresa_id, empresa_razon_social, empresa_rfc FROM " . $dbpfx . "empresas WHERE empresa_razon_social LIKE '%$razon%'";
		$matr1 = mysql_query($preg1) or die("ERROR: Fallo selección de empresas! " . $preg1);
		$fila1 = mysql_num_rows($matr1);
		if($fila1 > 0) {
			while($emp = mysql_fetch_array($matr1)) {
				$preg2 = "SELECT * FROM " . $dbpfx . "clientes WHERE cliente_empresa_id = '" . $emp['empresa_id'] . "'";
				$matr2 = mysql_query($preg2) or die("ERROR: Fallo selección de clientes! " . $preg2);
				while ($clie = mysql_fetch_array($matr2)) {
					$emprs[$clie['cliente_id']] = array($clie['cliente_empresa_id'], $emp['empresa_razon_social'], $emp['empresa_rfc'], $clie['cliente_nombre'], $clie['cliente_apellidos'], $clie['cliente_telefono1'], $clie['cliente_email']);
				}
			}
		}
	...
	if($razon != $_SESSION['exp']['razon'] && $razon != '') {
		$_SESSION['exp']['razon'] = strtoupper(preparar_entrada_bd($razon));
		$raz = explode(' ', $razon);
		$preg = "SELECT empresa_id, empresa_razon_social, empresa_rfc FROM " . $dbpfx . "empresas WHERE empresa_id > 0";
		foreach($raz as $k) {
			$preg .= " AND empresa_razon_social LIKE '%" . $k . "%'";
		}
		$matr = mysql_query($preg) or die("ERROR: Fallo selección de empresas! " . $preg);
		$fila = mysql_num_rows($matr);
			include('parciales/encabezado.php');
			echo '	<div id="body">';
			include('parciales/menu_inicio.php');
			echo '		<div id="principal">' ."\n";
			echo '			Por favor seleccione la Empresa con la Razón Social que corresponda.'."\n";
			echo '			<form action="reg-express.php?accion=express" method="post" enctype="multipart/form-data" name="empresa">'."\n";
			echo '			<table cellpadding="2" cellspacing="2" border="1">'."\n";
			echo '				<tr><td style="text-align:center;"># Empresa</td><td>Nombre Empresa</td><td>RFC</td><td>Selección</td></tr>'."\n";
			echo '				<tr><td style="text-align:center;">Nueva</td><td>' . $razon . '</td><td></td><td style="text-align:center;"><input type="radio" name="empresa_id" value="0" onchange="document.empresa.submit()" /><input type="hidden" name="orden_id" value="' . $orden_id . '"><input type="hidden" name="oid" value="' . $oid . '"></td></tr>'."\n";
			while($emp = mysql_fetch_array($matr)) {
				echo '				<tr><td style="text-align:center;">' . $emp['empresa_id'] . '</td><td>' . $emp['empresa_razon_social'] . '</td><td>' . $emp['empresa_rfc'] . '</td><td style="text-align:center;"><input type="radio" name="empresa_id" value="' . $emp['empresa_id'] . '" onchange="document.empresa.submit()" /></td></tr>'."\n";
			}
			echo '			</table></form>'."\n";
	} elseif($_SESSION['exp']['razon'] != '') {
		$preg1 = "SELECT * FROM " . $dbpfx . "clientes WHERE cliente_empresa_id = '" . $empresa_id . "'";
//		echo $preg1;
		...
		$fila1 = mysql_num_rows($matr1);
			...
			include('parciales/menu_inicio.php');
			echo '		<div id="principal">' ."\n";
			echo '			Por favor seleccione el Contacto que corresponda.'."\n";
			echo '			<form action="reg-express.php?accion=express" method="post" enctype="multipart/form-data" name="empresa">'."\n";
			echo '			<table cellpadding="2" cellspacing="2" border="1">'."\n";
			echo '				<tr><td style="text-align:center;"># Empresa</td><td>Nombre Empresa</td><td>RFC</td><td>Selección</td></tr>'."\n";
			echo '				<tr><td style="text-align:center;">Nueva</td><td>' . $razon . '</td><td></td><td style="text-align:center;"><input type="radio" name="empresa_id" value="0" onchange="document.empresa.submit()" /><input type="hidden" name="orden_id" value="' . $orden_id . '"><input type="hidden" name="oid" value="' . $oid . '"></td></tr>'."\n";
...
/*	if (!isset($razon) || $razon == '') {$razon = $nombre . ' ' . $apellidos;}
	$_SESSION['exp']['razon'] = strtoupper(preparar_entrada_bd($razon)); 
	$_SESSION['exp']['serie'] = strtoupper(limpiarString($serie));
	$_SESSION['exp']['marca'] = strtoupper(limpiarString($marca));
	$_SESSION['exp']['tipo'] = strtoupper(limpiarString($tipo));
	$_SESSION['exp']['subtipo'] = strtoupper(limpiarString($subtipo));
	$_SESSION['exp']['modelo'] = preparar_entrada_bd($modelo);
	$_SESSION['exp']['colores'] = strtoupper(limpiarString($colores));
	$_SESSION['exp']['puertas'] = strtoupper(limpiarString($puertas));
	$_SESSION['exp']['nombre'] = strtoupper(preparar_entrada_bd($nombre));
	$_SESSION['exp']['apellidos'] = strtoupper(preparar_entrada_bd($apellidos));
	...
	$_SESSION['exp']['email'] = preparar_entrada_bd($email);
	$_SESSION['exp']['telefono1'] = preparar_entrada_bd($telefono1);
	$_SESSION['exp']['telefono2'] = preparar_entrada_bd($telefono2);
	$_SESSION['exp']['movil'] = preparar_entrada_bd($movil);
	$_SESSION['exp']['movil2'] = preparar_entrada_bd($movil2);
	$_SESSION['exp']['asesor'] = preparar_entrada_bd($asesor);
	$_SESSION['exp']['servicio'] = preparar_entrada_bd($servicio);
	$_SESSION['exp']['garantia'] = preparar_entrada_bd($garantia);
	$_SESSION['exp']['categoria'] = preparar_entrada_bd($categoria);
	$_SESSION['exp']['orden_id'] = preparar_entrada_bd($orden_id);
	$_SESSION['exp']['espacio'] = $espacio;
//	$odometro = preparar_entrada_bd($odometro); $_SESSION['exp']['odometro'] = $odometro;
	$_SESSION['exp']['$torre'] = preparar_entrada_bd($torre);
