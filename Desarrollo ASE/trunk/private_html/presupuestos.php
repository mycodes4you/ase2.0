...
//  ----------------  obtener nombres de aseguradoras   ------------------- 
		...
		while ($aseg = mysql_fetch_array($arreglo)) {
			...
			$asehojapres[$aseg['aseguradora_id']] = $aseg['flota_hoja_pres'];
		...
		$ut[0] = $preciout;
		$asurt[0] = 1;
		$asehojapres[0] = 'hoja_pres_particular.php';
/*  ----------------  nombres de aseguradoras   ------------------- */

if (($accion==='insertar') || ($accion==='actualizar') || ($accion==='avaluo') || ($accion==='valuar') || ($accion==='mod_avaluo') || ($accion==='confcancelar') || $accion==='termcotizar' || $accion==='imprimepres'  || $accion==='envio' || $accion==='compara' || $accion==='presupuesto' || $accion==='imprimeaut' || $accion==='trabmodificar' || $accion==='trabmodifica') {
...
if (($accion==="crear") || ($accion==="complemento") || ($accion==="adicional")) {
	...
	<table cellpadding="0" cellspacing="0" border="0" class="izquierda">
		<tr><td colspan="2" style="text-align:left;"><span style="font-weight:bold; font-size:1.2em;">' . $veh['completo'] . ' <strong>OT ' . $orden_id . '</strong></span></td></tr>';
...
//		echo $pregunta2;
 	...
 	if ($orden['orden_servicio']=="4") {
		...
		<tr><td>' . $lang['NumSin'] . '</td><td style="text-align:left;">' . $sub['sub_reporte'] . '<input type="hidden" name="reporte" value="' . $sub['sub_reporte'] . '" /><input type="hidden" name="poliza" value="' . $sub['sub_poliza'] . '" /></td></tr>';
			...
			if($veh['flota'] > 0) {
				echo '		<tr><td colspan="2">' . $lang['AsegConv'] . ': '."\n";
				echo '			<input type="hidden" name="aseguradora" value="' . $veh['flota'] . '" />' . constant('ASEGURADORA_NIC_' . $veh['flota']);
				echo '		</td></tr>';
				echo '		<tr class="claro"><td colspan="2">' . $lang['NumSin'] . ' <input type="text" name="reporte" size="30" maxlength="30" value="';
				if (isset($_SESSION['pres']['reporte'])) { echo $_SESSION['pres']['reporte']; }
				echo '" /></td></tr>'."\n";
			} else {
				echo '		<tr><td>' . $lang['AsegConv'] . ': </td><td style="text-align:left;">'."\n";
...
//		if ($pagadedu < '1' && $adicional != '1') {$error = 'si'; $mensaje .='Debe indicar si o no paga deducible.<br>';}
	...
	if($orden_estatus >= '30') { $error = 'si'; $mensaje .= 'No se puede crear tarea en el estatus actual de la orden.<br>'; }

	if ($error === 'no') {
		if (isset($paquetes) && $paquetes!='Seleccione') {
			$preg3 = "SELECT * FROM " . $dbpfx . "paquetes WHERE paq_id='" . $paquetes . "'";
			$matr3 = mysql_query($preg3) or die("ERROR: Fallo selección de paq_prods!");
			$desc = mysql_fetch_array($matr3);
			$pregs = "SELECT orden_sitio_actual FROM " . $dbpfx . "ordenes WHERE orden_id = '" . $orden_id . "'";
			$matrs = mysql_query($pregs) or die("ERROR: Fallo selección de sitio actual! 258 " . $pregs);
			$op_ubic = mysql_fetch_array($matrs);
			$opubic = $op_ubic['orden_sitio_actual'];
			...
			while($tarea = mysql_fetch_array($matr4)) {
				...
				while($paqs = mysql_fetch_array($matr0)) {
					//echo 'paquete<br>';
					$solicitar_cot = 0;
					$preg1 = "SELECT prod_cant_cotizar, prod_marca, prod_prov_id, prod_costo, prod_codigo, prod_nombre, prod_tangible, prod_precio FROM " . $dbpfx . "productos WHERE prod_id='" . $paqs['pc_prod_id'] . "'";
					//	echo $preg1.'<br>';
					$matr1 = mysql_query($preg1) or die("ERROR: Fallo selección de paq_prods! " . $preg1);
					while($prods = mysql_fetch_array($matr1)) {
						// --- Obtenemos los disponibles del almacén correspondiente ---
						$pregdis = "SELECT prod_disponibles FROM " . $dbpfx . "prod_existencias WHERE prod_almacen = '" . $opubic . "' AND prod_id = '" . $prods . "'";
						$matrdis = mysql_query($pregdis) or die("ERROR: Fallo selección de productos! 289 " . $pregdis);
						$prodis = mysql_fetch_array($matrdis);
						// --- Atajo para no cambiar $prods['prod_cantidad_disponible'] pero se deberá ajustar con calma ---
						if($prodis['prod_disponibles'] > 0) { $prods['prod_cantidad_disponible'] = $prodis['prod_disponibles']; }
						else { $prods['prod_cantidad_disponible'] = 0; }
						...
						$sql_data_array = [
							...
							'op_reservado' => $paqs['pc_prod_cant'],
						...
						if($paqs['pc_prod_cant'] > $prods['prod_cantidad_disponible'] && $prods['prod_tangible'] > '0') {
							...
							$parme = " prod_id = '" . $paqs['pc_prod_id'] . "' AND prod_almacen = '" . $opubic . "' ";
							$sqdat = ['prod_disponibles' => '0'];
							ejecutar_db($dbpfx . 'prod_existencias', $sqdat, 'actualizar', $parme);
							unset($sqdat);
							...
							$parme = " prod_id = '" . $paqs['pc_prod_id'] . "' AND prod_almacen = '" . $opubic . "' ";
							$sqdat = ['prod_disponibles' => ($prods['prod_cantidad_disponible'] - $paqs['pc_prod_cant'])];
							ejecutar_db($dbpfx . 'prod_existencias', $sqdat, 'actualizar', $parme);
							unset($sqdat);
						}
						...
						$item++;
						if($prods['prod_tangible'] > '0') {
							// --- Crear registro de productos pendientes ---
							$sql_data_array = [
								'prod_id' => $paqs['pc_prod_id'],
								'prods_pendiente_requeridos' => $paqs['pc_prod_cant'],
								'prods_pendiente_surtidos' => $prods['prod_cantidad_disponible'],
								'prods_pendiente_adeudos' => ($paqs['pc_prod_cant'] - $prods['prod_cantidad_disponible']),
						...
						if($solicitar_cot == 1) {
							...
							$preg_proveedor = "SELECT prov_email, prov_razon_social, prov_rfc FROM " . $dbpfx . "proveedores WHERE prov_id = '" . $prods['prod_prov_id'] . "' AND prov_env_ped = '1' AND prov_activo = '1'";
							$matr_proveedor = mysql_query($preg_proveedor) or die("ERROR: Fallo selección de proveedor! " . $preg_proveedor);
							$filaprov = mysql_num_rows($matr_proveedor);
							if($filaprov == 1) {
								$prov = mysql_fetch_assoc($matr_proveedor);
								...
								if($qv_activo == 1) {
									...
									$xml .= '		<OT orden_id="Almacen" marca="n/a" tipo="n/a" color="n/a" vin="n/a" modelo="n/a" rfc="' . $prov['prov_rfc'] . '">'."\n";
								...
								if($envcotalmacen01 == 1) {
								// --- Enviar correo de cotización y crear cotización ---
									unset($sql_data);
									$sql_data = [
										'prod_id' => $paqs['pc_prod_id'], 
...
elseif ($accion==='actualizar') {
	...
	if(validaAcceso('1045010', $dbpfx) == '1' || $_SESSION['rol02'] == '1' || $_SESSION['rol04'] == '1' || $_SESSION['rol06']=='1') {
		// Acceso autorizado
	} else {
		redirigir('usuarios.php?mensaje=Acceso NO autorizado ingresar Usuario y Clave correcta');
	}
   ...
   if ($error === 'no') {
		$sql_data_array = array('sub_descripcion' => $nvadescripcion);
		if($estatus == '112') {
			...
			if($fila > 0) {
				...
				$mensaje .= 'Tarea Terminada ya facturada o con recibo de destajo. No se cambió estatus.<br>';
			}
		} else {
			$sql_data_array['sub_estatus'] = $nvoestatus;
		}
		...
		$comentario = '';
		if($nvaaseguradora != $aseguradora && $flota < 1) { $comentario .= 'Cliente '; }
		if($nvadescripcion != $descripcion) { $comentario .= 'Descripción '; }
		if($nvoestatus != $estatus) { $comentario .= 'Estatus '; }
		bitacora($orden_id, 'Cambio de ' . $comentario . 'para la Tarea ' . $sub_orden_id, $dbpfx, 'Cambié ' . $comentario . 'para la Tarea ' . $sub_orden_id . ' de ' . constant('NOMBRE_AREA_' . $area), 2, $sub_orden_id);
		actualiza_orden($orden_id, $dbpfx);
...
elseif ($accion==='modificar') {
	...
	if(validaAcceso('1045010', $dbpfx) == '1' || $_SESSION['rol02'] == '1' || $_SESSION['rol04'] == '1' || $_SESSION['rol05'] == '1' || $_SESSION['rol06'] == '1') {
		// Acceso autorizado
	} else {
		 redirigir('usuarios.php?mensaje=Acceso NO autorizado ingresar Usuario y Clave correcta');
	}
...
//	echo 'Estamos en la sección valuar';
	...
	echo '					<tr><td>Descripción de tarea: </td><td><textarea name="nvadescripcion" cols="60" rows="3" >' . $sub_orden['sub_descripcion'] . '</textarea></td></tr>'."\n";

	// ------ Determina quien tiene permiso de cambiar estatus de las tareas ---
	unset($nvoe);
	if(validaAcceso('1045010', $dbpfx) == '1' || $_SESSION['rol06'] == '1') {
		$acceso[30] = 1;
	}
	if(validaAcceso('1045010', $dbpfx) == '1' || $_SESSION['rol07'] == '1') {
		$acceso[40] = 1;
	}
	if(validaAcceso('1045010', $dbpfx) == '1' || $_SESSION['rol05'] == '1') {
		$acceso[20] = 1;
	}
	if(validaAcceso('1045010', $dbpfx) == '1' || $_SESSION['rol04'] == '1') {
		$acceso[15] = 1;
	}
	if(validaAcceso('1045010', $dbpfx) == '1' || $_SESSION['rol02'] == '1') {
		$acceso[10] = 1;
	}

	// --- Estatus permitidos para cambiar -----
	if($sub_orden['sub_estatus'] == 124 && $acceso[30] == 1) {
		$nvoe = array(101); $derecho = 'Asesor de Servicio';
	} elseif($sub_orden['sub_estatus'] == 128 && $acceso[40] == 1) {
		$nvoe = array(101,124); $derecho = 'Presupuestador';
	} elseif($sub_orden['sub_estatus'] == 129 && $acceso[20] == 1) {
		$nvoe = array(101,124,128); $derecho = 'Valuador';
	} elseif($sub_orden['sub_estatus'] == 120 && $acceso[20] == 1) {
		$nvoe = array(101,124,128,129); $derecho = 'Valuador';
	} elseif($sub_orden['sub_estatus'] == 102 && $acceso[20] == 1) {
		$nvoe = array(101,124,128,129,120); $derecho = 'Valuador';
	} elseif(($sub_orden['sub_estatus'] == 103 || $sub_orden['sub_estatus'] == 104) && ($acceso[15] == 1 || $acceso[10] == 1)) {
		$nvoe = array(101,124,128,129,120,102); $derecho = 'Jefe de Taller y Gerente';
	} elseif($sub_orden['sub_estatus'] == 111 && $acceso[15] == 1) {
		$nvoe = array(101,124,128,129,120,102,104); $derecho = 'Jefe de Taller y Gerente';
	} elseif($sub_orden['sub_estatus'] == 121 && $acceso[15] == 1) {
		$nvoe = array(101,124,128,129,120,102,104,111); $derecho = 'Jefe de Taller y Gerente';
	} elseif($sub_orden['sub_estatus'] == 112  && $acceso[10] == 1) {
		$nvoe = array(101,124,128,129,120,102,104,111,121); $derecho = 'Gerente';
	} else {
...
elseif ($accion === 'mover_items') {
	...
	$matr3 = mysql_query($preg3) or die ('Fallo !!!!!'.$preg3);
...
elseif ($accion === 'mover') {
	...
	//secho 'Mover a tarea ' . $suborden_id . '<br>';
	// --- Consultarn el area de la tarea ---
...
elseif ($accion==='valuar') {
	...
	if (validaAcceso('1045025', $dbpfx) == '1' || $_SESSION['rol05']=='1' || (($_SESSION['rol04']=='1' || $_SESSION['rol06']=='1' || $_SESSION['rol07']=='1') && $sub_orden['sub_aseguradora'] == '0')) {
		...
		$_SESSION['msjerror'] = 'Acceso NO autorizado';
		 redirigir('presupuestos.php?accion=consultar&orden_id=' . $sub_orden['orden_id']);
	}

	// --- Debido a la autorización electrónica, no se permite modificar tareas una vez autorizadas ---
	if(($sub_orden['sub_estatus'] >= '102' && $sub_orden['sub_estatus'] <= '112') || $sub_orden['sub_estatus'] == '121') {
		$_SESSION['msjerror'] = 'No se puede modificar una tarea autorizada por el cliente.<br>';
		 redirigir('presupuestos.php?accion=consultar&orden_id=' . $sub_orden['orden_id']);
	}
...
elseif ($accion==='cesta') {
	...
	$sub_orden = mysql_fetch_array($matriz);
	$veh = datosVehiculo($sub_orden['orden_id'], $dbpfx);
	$pregs = "SELECT orden_sitio_actual FROM " . $dbpfx . "ordenes WHERE orden_id = '" . $sub_orden['orden_id']. "'";
	$matrs = mysql_query($pregs) or die("ERROR: Fallo selección de sitio actual! 1559 " . $pregs);
	$op_ubic = mysql_fetch_array($matrs);
	$opubic = $op_ubic['orden_sitio_actual'];

	echo '			<div class="page-content">'."\n";

	echo '				<div class="row"><div class="col-md-10"><div class="content-box-header"><div class="panel-title">
					<h2>Seleccionar Refacciones, Consumibles y Mano de Obra del Almacén</h2>
				</div></div></div></div>'."\n";
	echo '				<div class="row"><div class="col-md-10 shadow-box"><div class="form-group"><div class="col-md-12">'."\n";
	echo '					<form action="presupuestos.php?accion=cesta" method="post" enctype="multipart/form-data">'."\n";
	echo '					<div class="row">'."\n";
	echo '						<div class="col-sm-10">
						<p style="font-size:1.2em; font-weight:bold;">' . $veh['marca'] . ' ' . $veh['tipo'] . ' ' . $veh['color'] . $lang['Placas'] . $veh['placas'] . '</p>'."\n";
	echo '						</div></div>'."\n";
	if($preaut==1) { echo '<input type="hidden" name="preaut" value="1" />'; }
	echo '					<div class="row">
						<div class="col-sm-2">Almacén Asignado:</div>
						<div class="col-sm-4">' . $ubicaciones[$opubic] . '</div>
					</div>
					<div class="row">
						<div class="col-sm-2">Buscar por código:</div>
						<div class="col-sm-4"><input type="text" name="codigo" value="' . $codigo . '" size="15"></div>
					</div>
					<div class="row">
						<div class="col-sm-2">Buscar por nombre:</div>
						<div class="col-sm-4"><input type="text" name="nombre" value="' . $nombre . '" size="15"></div>
					</div>
					<div class="row">
						<div class="col-sm-6"><input name="Enviar" value="Enviar" type="submit">
							<input type="hidden" name="sub_orden_id" value="' . $sub_orden_id . '">
...
//	echo $paginas;
	$preg3 = "SELECT prod_id, prod_codigo, prod_marca, prod_nombre, prod_precio, prod_precioint, prod_tangible FROM " . $dbpfx . "productos WHERE prod_activo='1' AND prod_tangible < '3' ";
	$preg3 = $preg3 . $preg;
	$preg3 .= "ORDER BY prod_nombre LIMIT " . $inicial . ", " . $renglones;
	$matr3 = mysql_query($preg3) or die("ERROR: Fallo selección de productos! 1234 " . $preg3);
//	echo $preg3; , e., e.prod_almacen
	$preg4 = "SELECT prod_id, prod_disponibles FROM " . $dbpfx . "prod_existencias WHERE prod_almacen = '" . $opubic . "'";
	$matr4 = mysql_query($preg4) or die("ERROR: Fallo selección de productos! 1237 " . $preg4);
	while($prode = mysql_fetch_array($matr4)) {
		$pexi[$prode['prod_id']] = $prode['prod_disponibles'];
	}
	...
	echo '						<div class="row">
							...
							<div class="col-sm-1">
								<a href="presupuestos.php?accion=cesta&sub_orden_id=' . $sub_orden_id . '&pagina=0&cpres=' . $cpres .'&nombre=' . $nombre . '&almacen=' . $almacen . '">Inicio</a>&nbsp;';
	if($pagina > 0) {
		$url = $pagina - 1;
	...
	echo '				<div class="row">
					<div class="col-md-10">'."\n";
	echo '						<div class="row">
							<div class="col-sm-1">Prod Id</div>
							<div class="col-sm-1">Código</div>
							<div class="col-sm-5">Nombre</div>
							<div class="col-sm-1">Marca</div>
							<div class="col-sm-1 cen">Disponibles</div>
							<div class="col-sm-1">Precio Público</div>
							<div class="col-sm-1">Precio Interno</div>
							<div class="col-sm-1 cen">Cantidad</div>
						</div>'."\n";
	$fondo = 'obscuro';
	while($prods = mysql_fetch_array($matr3)) {
		echo '							<div class="row ' . $fondo . '">
							<div class="col-sm-1">' . $prods['prod_id'] . '</div>
							<div class="col-sm-1">' . $prods['prod_codigo'] . '<input type="hidden" name="prod_codigo[' . $prods['prod_id'] . ']" value="' . $prods['prod_codigo'] . '" /></div>
							<div class="col-sm-5">' . $prods['prod_nombre'] . '<input type="hidden" name="prod_nombre[' . $prods['prod_id'] . ']" value="' . $prods['prod_nombre'] . '" /></div>
							<div class="col-sm-1">' . $prods['prod_marca'] . '</div>
							<div class="col-sm-1 cen">';
							if($pexi[$prods['prod_id']] > 0) { echo $pexi[$prods['prod_id']]; }
							else { echo '0'; }
							echo '<input type="hidden" name="prod_disponible[' . $prods['prod_id'] . ']" value="' . $pexi[$prods['prod_id']] . '" /></div>
							<div class="col-sm-1 der">' . number_format($prods['prod_precio'],2) . '</div>
							<div class="col-sm-1 der">' . number_format($prods['prod_precioint'],2) . '</div>
							<div class="col-sm-1">
								<input type="text" name="prod_cantidad[' . $prods['prod_id'] . ']" size="2" />
								<input type="hidden" name="prod_id[' . $prods['prod_id'] . ']" value="' . $prods['prod_id'] . '" />
								<input type="hidden" name="prod_precio[' . $prods['prod_id'] . ']" value="' . $prods['prod_precio'] . '" />
								<input type="hidden" name="prod_costo[' . $prods['prod_id'] . ']" value="' . $prods['prod_precioint'] . '" />
								<input type="hidden" name="prod_tangible[' . $prods['prod_id'] . ']" value="' . $prods['prod_tangible'] . '" />
							</div>
						</div>'."\n";
					...
					<div class="col-md-10">
						<div class="row">
							<div class="col-sm-6"><input name="Enviar" value="Enviar" type="submit">
								...
								<input type="hidden" name="area" value="' . $sub_orden['sub_area'] . '" />
								<input type="hidden" name="cpres" value="' . $cpres . '" />
								<input type="hidden" name="sitioact" value="' . $opubic . '" />
...
elseif ($accion==='cancelar') {
	...
	if(validaAcceso('1045045', $dbpfx) == '1' || ($solovalacc != 1 && ($_SESSION['rol02'] == '1' || $_SESSION['rol05'] == '1' || $_SESSION['rol06'] == '1'))) {	
		// Acceso autorizado
	} else {
		$_SESSION['msjerror'] = $lang['Acceso No Autorizado'];
		redirigir('presupuestos.php?accion=consultar&sub_orden_id=' . $sub_orden_id);
	...
	$error = 'no';
	$preg1 = "SELECT op_surtidos, op_nombre, op_pedido FROM " . $dbpfx . "orden_productos WHERE sub_orden_id = '$sub_orden_id' AND op_tangible < 3";
	...
	while($op = mysql_fetch_array($matr1)) {
		...
		if($op['op_surtidos'] > 0) {
			$error = 'si'; $falla2 = 1;
			$refsurt .= $op['op_surtidos'] . ' ' . $op['op_nombre'] . '<br>';
		}
	}
	...
	if($falla2 == 1) {
		echo '			<p class="alerta">La Tarea tiene refacciones entregadas a operadores:<br><br>' . $refsurt . '<br>mismos que debe(n) de ser devueltos para poder eliminar esta Tarea.</p>'."\n";
	}

	$pregunta = "SELECT recibo_id, fact_id, orden_id, sub_area, sub_aseguradora, sub_presupuesto FROM " . $dbpfx . "subordenes WHERE sub_orden_id = '$sub_orden_id'";
	$matriz = mysql_query($pregunta) or die("ERROR: Fallo selección de tarea! " . $pregunta);
	$sub_orden = mysql_fetch_array($matriz);
...
elseif ($accion==='confcancelar') {
	...
	if ($confirmar=="Confirmar") {
		// --- Consultar si los productos fueron surtidos desde almacén ---
		$preg_paq = "SELECT op_id, prod_id, op_surtidos, op_nombre, op_recibidos, op_surtidos FROM " . $dbpfx . "orden_productos WHERE sub_orden_id = '$sub_orden_id' AND prod_id > 0";
		...
		$pregs = "SELECT orden_sitio_actual FROM " . $dbpfx . "ordenes WHERE orden_id = '" . $orden_id . "'";
		$matrs = mysql_query($pregs) or die("ERROR: Fallo selección de sitio actual! 1553 " . $pregs);
		$op_ubic = mysql_fetch_array($matrs);
		$opubic = $op_ubic['orden_sitio_actual'];

		if($fila_paq > 0) { // --- Regresar los disponibles al almacen ---
			while($paq_ser = mysql_fetch_array($matr_paq)) {
				// --- Consultar si hay reservados para cancelarlos -----
				...
				if($num_pendientes > 0) { // --- Cancelar los reservados ---
					while($pendientes = mysql_fetch_array($matr_pendientes)) {
						// --- Reponer existencias ---
						$suma = "UPDATE " . $dbpfx . "prod_existencias SET prod_disponibles = prod_disponibles + " . $pendientes['prods_pendiente_surtidos'] . " WHERE prod_id = '" . $paq_ser['prod_id'] . "' AND prod_almacen = '" . $opubic . "'";
						//echo $suma . '<br>';
						...
						ejecutar_db($dbpfx . 'prods_pendientes', '', 'eliminar', $parme);
					}
				}
			}
		}
...
// ------ Localizamos el archivo con el algoritmo a utilizar para esta aseguradora
	$pregaseg = "SELECT a.algoritmo_pres, s.sub_siniestro, s.sub_aseguradora FROM " . $dbpfx . "aseguradoras a, " . $dbpfx . "subordenes s WHERE s.sub_orden_id = '$sub_orden_id' AND s.sub_aseguradora = a.aseguradora_id";
	$matraseg = mysql_query($pregaseg) or die("ERROR: No se localizó la aseguradora de esta tarea! " . $pregaseg);
	$filaaseg = mysql_num_rows($matraseg);
	if($filaaseg == 0) {
		if(is_array($valor['AlgoritmoDefault'])) {
...
/*
	if (validaAcceso('1045070', $dbpfx) == '1' || ($solovalacc != 1 && ($_SESSION['rol02']=='1' || $_SESSION['rol04']=='1' || $_SESSION['rol05']=='1' || $_SESSION['rol06']=='1' || $_SESSION['rol12']=='1' || $_SESSION['rol14']=='1'))) {
		$mensaje = 'Acceso autorizado';
	} else {
		$_SESSION['msjerror'] = 'Sin acceso';
		redirigir('usuarios.php?mensaje=Acceso sólo para Cotizador, ingresar Usuario y Clave correcta');
	}
*/
	$mensaje = '';
...
//		echo $pregunta;
		...
		$pregu = "SELECT usuario, nombre, apellidos, aseg FROM " . $dbpfx . "usuarios WHERE activo = '1'";
		$matru = mysql_query($pregu) or die("ERROR: Fallo selección de usuarios!");
		while ($usu = mysql_fetch_array($matru)) {
			$usr[$usu['usuario']]['nombre'] = $usu['nombre'] . ' ' . $usu['apellidos'];
			$usraseg[$usu['usuario']]['aseg'] = $usu['aseg'];
		}
...
//		if($aseguradora > '0') { $preg .= "o.op_pres = '1'"; } else { $preg .= "o.op_pres IS NULL"; }
		...
		if ($num_cols > 0) {
			...
			if (file_exists('particular/' . $asehojapres[$aseguradora])) {
	 			//echo "El fichero hoja_pres existe en particular";
				include('particular/' . $asehojapres[$aseguradora]);
			} else {
...
elseif ($accion==="trabmodificar") {
	...
	// --- Modifica todas las tareas de un siniestro específico o todas las particulares.
	...
	echo '				<table cellpadding="0" cellspacing="0" border="0" class="agrega">
					<tr><td colspan="2" style="text-align:left;">' . $veh['marca'] . ' ' . $veh['tipo'] . ' ' . $veh['color'] . ' ' . $lang['Placas'] . $veh['placas'] .'</td></tr>'. "\n";
	...
	if($veh['flota'] < 1) {
		...
		while($aseg3 = mysql_fetch_array($matriz3)) {
			echo '							<option value="' . $aseg3['aseguradora_id'] . '" ';
			if($aseg3['aseguradora_id'] == $aseguradora) { echo 'selected ';}
			echo '>' . $aseg3['aseguradora_nic'] . '</option>'."\n";
		}
		echo '						</select>
					</td></tr>'."\n";
	} else {
		echo '					<tr class="cabeza_tabla"><td colspan="2"><input type="hidden" name="aseguradora" value="' . $aseguradora . '" /></td></tr>'."\n";
	}
	echo '				<tr><td>Imagen escaneada en JPG de la orden de admisión</td><td style="text-align:left;"><input type="file" name="orden_adm" size="30" /></td></tr>
				...
				<tr><td>Número de reporte de Cercotec: </td><td style="text-align:left;"><input type="text" name="nvoreporte" size="20" maxlength="40" value="' . $reporte . '" /></td></tr>'."\n";
//				<tr><td>Número de póliza de aseguradora: </td><td style="text-align:left;"><input type="text" name="poliza" size="20" maxlength="40" value="' . $poliza . '" /></td></tr>'."\n";
//	echo '				<tr><td>Monto de deducible: </td><td style="text-align:left;"><input type="text" name="deducible" size="20" value="' . number_format($deducible,2) . '" /></td></tr>'."\n";
	...
	echo '<a href="presupuestos.php?accion=imprimeaut&sin=' . $reporte . '&orden_id=' . $orden_id . '"><img src="idiomas/' . $idioma . '/imagenes/imprimir-presupuesto-h.png" alt="" /></a>';
	echo '</td></tr>'."\n";
...
elseif ($accion==='trabmodifica') {
	...
	$error = 'no';
	$mensaje = 'No se actualizó el Trabajo.<br>';
	...
	if ($error === 'no') {
		...
		if($aseguradora > '0') {
			$sql_data_array['sub_reporte'] = $nvoreporte;
			$sql_data_array['sub_deducible'] = $deducible;
			$sql_data_array['sub_poliza'] = $poliza;
			$sql_data_array['sub_doc_adm'] = $nombre_archivo;
			$sql_data_array['sub_siniestro'] = '1';
			$sql_orden['orden_servicio'] = '4';
			$quien = 'Siniestro ' . $nvoreporte;
		} else {
			$sql_orden['orden_servicio'] = '3';
			$sql_data_array['sub_reporte'] = '0';
			$sql_data_array['sub_siniestro'] = '0';
			$quien = 'Trabajo Particular';
		}
		...
		bitacora($orden_id, 'Cambio de datos generales y deducible del ' . $previo . ' a ' . $quien, $dbpfx);
